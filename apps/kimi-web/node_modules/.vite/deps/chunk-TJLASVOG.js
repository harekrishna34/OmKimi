...
// ../../node_modules/.pnpm/stream-markdown-parser@1.1.7/node_modules/stream-markdown-parser/dist/index.js
...
function ins_plugin(md) {
  ...
  function postProcess$2(state, delimiters) {
    ...
    while (loneMarkers.length) {
      ...
      let j2 = i2 + 1;
      while (j2 < state.tokens.length && state.tokens[j2].type === "ins_close") j2++;
      j2--;
      if (i2 !== j2) {
...
function ins_plugin$1(md) {
  ...
  function postProcess$2(state, delimiters) {
    ...
    while (loneMarkers.length) {
      const i2 = loneMarkers.pop();
      let j2 = i2 + 1;
      while (j2 < state.tokens.length && state.tokens[j2].type === "mark_close") j2++;
      j2--;
      if (i2 !== j2) {
...
var decode_data_xml_default = new Uint16Array("Ȁaglq	\x1Bɭ\0\0p;䀦os;䀧t;䀾t;䀼uot;䀢".split("").map((c2) => c2.charCodeAt(0)));
var _a;
    ...
    if (!ipv6Hostname) {
      ...
      for (let i2 = 0, l2 = hostparts.length; i2 < l2; i2++) {
        ...
        if (!part.match(hostnamePartPattern)) {
          let newpart = "";
          for (let j2 = 0, k2 = part.length; j2 < k2; j2++) if (part.charCodeAt(j2) > 127) newpart += "x";
          else newpart += part[j2];
...
function isValidEntityCode(c2) {
  if (c2 >= 55296 && c2 <= 57343) return false;
  if (c2 >= 64976 && c2 <= 65007) return false;
  if ((c2 & 65535) === 65535 || (c2 & 65535) === 65534) return false;
  if (c2 >= 0 && c2 <= 8) return false;
  if (c2 === 11) return false;
  if (c2 >= 14 && c2 <= 31) return false;
  if (c2 >= 127 && c2 <= 159) return false;
  if (c2 > 1114111) return false;
  return true;
}
...
var FOOTNOTE_DEF_RE = /(?:^|\n)[ \t]{0,3}\[\^[^\]\n]+\]:/m;
var ABBR_DEF_RE = /(?:^|\n)[ \t]{0,3}\*\[[^\]\n]+\]:/m;
var REFERENCE_DEF_RE = /(?:^|\n)[ \t]{0,3}\[(?!\^)(?:\\[\s\S]|[^\]\\[])+\][ \t]*:/m;
var GLOBAL_STATE_ENV_KEYS = [
  "references",
  "footnotes",
  "abbreviations",
  "abbr",
...
var GLOBAL_STATE_ENV_MARKER = Symbol.for("markdown-it-ts.global-state");
...
function snapshotValueEquals(left, right) {
  if (Array.isArray(left) || Array.isArray(right)) {
    ...
    for (let i2 = 0; i2 < left.length; i2++) if (!snapshotValueEquals(left[i2], right[i2])) return false;
    return true;
  }
...
function restoreSnapshotValue(target, snapshot) {
  if (Array.isArray(target) && Array.isArray(snapshot)) {
    ...
    for (let i2 = 0; i2 < snapshot.length; i2++) target[i2] = cloneSnapshotValue(snapshot[i2]);
    return target;
  }
...
function getKnownGlobalMarkdownState(env) {
  var _a2;
  return ((_a2 = getMarker(env)) == null ? void 0 : _a2.reason) ?? null;
}
...
var MDTS_DIAGNOSTICS = Symbol.for("markdown-it-ts.diagnostics");
...
LinkifyIt.prototype.test = function test(text$1) {
  ...
  if (this.re.schema_test.test(text$1)) {
    ...
    re.lastIndex = 0;
    while ((m2 = re.exec(text$1)) !== null) if (this.testSchemaAt(text$1, m2[2], re.lastIndex)) return true;
  }
...
LinkifyIt.prototype.match = function match(text$1) {
  ...
  function choose(a2, b2) {
    if (!a2) return b2;
    if (!b2) return a2;
    if (a2.index !== b2.index) return a2.index < b2.index ? a2 : b2;
  ...
  if (this.re.schema_test.test(text$1)) {
    re = this.re.schema_search;
    re.lastIndex = 0;
    while ((m2 = re.exec(text$1)) !== null) {
      len = this.testSchemaAt(text$1, m2[2], re.lastIndex);
  ...
  if (this.__opts__.fuzzyLink && this.__compiled__["http:"]) {
    ...
    re.lastIndex = 0;
    while ((m2 = re.exec(text$1)) !== null) type_fuzzy_link.push({
      schema: "",
      index: m2.index + m2[1].length,
  ...
  if (this.__opts__.fuzzyEmail && this.__compiled__["mailto:"]) {
    ...
    re.lastIndex = 0;
    while ((m2 = re.exec(text$1)) !== null) type_fuzzy_email.push({
      schema: "mailto:",
      index: m2.index + m2[1].length,
      lastIndex: m2.index + m2[0].length
    });
...
LinkifyIt.prototype.matchAtStart = function matchAtStart(text$1) {
  if (!text$1.length) return null;
  const m2 = this.re.schema_at_start.exec(text$1);
  if (!m2) return null;
  const len = this.testSchemaAt(text$1, m2[2], m2[0].length);
  if (!len) return null;
  const match2 = new Match(text$1, m2[2], m2.index + m2[1].length, m2.index + m2[0].length + len);
...
var require_punycode = __commonJS({ "../../node_modules/.pnpm/punycode.js@2.3.1/node_modules/punycode.js/punycode.js": ((exports, module) => {
  ...
  const adapt = function(delta, numPoints, firstTime) {
    ...
    for (; delta > baseMinusTMin * tMax >> 1; k2 += base) delta = floor(delta / baseMinusTMin);
  ...
  const decode = function(input) {
    ...
    for (let j2 = 0; j2 < basic; ++j2) {
      if (input.charCodeAt(j2) >= 128) error("not-basic");
    ...
    for (let index = basic > 0 ? basic + 1 : 0; index < inputLength; ) {
      ...
      for (let w2 = 1, k2 = base; ; k2 += base) {
        ...
        if (digit > floor((maxInt - i2) / w2)) error("overflow");
        i2 += digit * w2;
        const t2 = k2 <= bias ? tMin : k2 >= bias + tMax ? tMax : k2 - bias;
        if (digit < t2) break;
        const baseMinusT = base - t2;
        if (w2 > floor(maxInt / baseMinusT)) error("overflow");
        w2 *= baseMinusT;
      }
  ...
  const encode = function(input) {
    ...
    while (handledCPCount < inputLength) {
      let m2 = maxInt;
      for (const currentValue of input) if (currentValue >= n2 && currentValue < m2) m2 = currentValue;
      const handledCPCountPlusOne = handledCPCount + 1;
      if (m2 - n2 > floor((maxInt - delta) / handledCPCountPlusOne)) error("overflow");
...
function emphasis_tokenize(state, silent) {
  ...
  for (let i2 = 0; i2 < scannedLength; i2++) {
    const token = state.push("text", "", 0);
...
function postProcess$1(state, delimiters) {
  ...
  for (let i2 = max - 1; i2 >= 0; i2--) {
    ...
    const isStrong = i2 > 0 && delimiters[i2 - 1].end === startDelim.end + 1 && delimiters[i2 - 1].marker === marker && delimiters[i2 - 1].token === startTokenIdx - 1 && delimiters[startDelim.end + 1].token === endTokenIdx + 1;
    const markerChar = marker === 42 ? MARKER_ASTERISK : MARKER_UNDERSCORE;
    ...
    if (isStrong) {
      tokens[delimiters[i2 - 1].token].content = "";
...
function postProcess(state, delimiters) {
  ...
  while (loneMarkers.length) {
    const i2 = loneMarkers.pop();
    let j2 = i2 + 1;
    while (j2 < state.tokens.length && state.tokens[j2].type === "s_close") j2++;
    j2--;
    if (i2 !== j2) {
      token = state.tokens[j2];
      state.tokens[j2] = state.tokens[i2];
...
var InlineRuler = class {
  ...
  */
  ...
  at(name, fn, options) {
    ...
    if ((options == null ? void 0 : options.alt) !== void 0) this.rules[index].alt = options.alt;
    this.invalidateCache();
  }
...
var StateInline = class {
  constructor(src, md, env, outTokens) {
    ...
    __publicField(this, "linkLabelNoCloseFrom");
...
var INLINE_TERMINATOR_RE$1 = /[\n!#$%&*+\-:<=>@[\]\\^_`{}~]/;
function runDefaultRule(state, silent) {
  switch (state.src.charCodeAt(state.pos)) {
    case 10:
      return newline_default(state, silent);
    case 33:
      return image_default(state, silent);
    case 38:
      return entity_default(state, silent);
    case 42:
    case 95:
      return emphasis.tokenize(state, silent);
    case 58:
      return state.md.options.linkify && linkify$1(state, silent);
    case 60:
      return autolink_default(state, silent) || html_inline_default(state, silent);
    case 91:
      return link_default(state, silent);
    case 92:
      return escape_default(state, silent);
    case 96:
      return backticks_default(state, silent);
    case 126:
      return strikethrough.tokenize(state, silent);
    default:
      return text_default(state, silent);
  }
}
...
var ParserInline = class {
  constructor() {
    ...
    __publicField(this, "defaultRulerVersion");
    __publicField(this, "defaultRuler2Version");
...
function inline(state) {
  var _a2, _b, _c;
  const tokens = state.tokens;
  const canUsePlainTextFastPath = !!((_c = (_b = (_a2 = state.md) == null ? void 0 : _a2.inline) == null ? void 0 : _b.isDefaultRuleset) == null ? void 0 : _c.call(_b));
...
function trimCjkPrefixFromFuzzyLink(linkify$2, link$1) {
  ...
  for (let offset = 1; offset < link$1.raw.length; offset++) {
    ...
    const candidate = (_a2 = linkify$2.match(suffix)) == null ? void 0 : _a2[0];
...
function linkify(state) {
  ...
  for (let j2 = 0; j2 < blockTokens.length; j2++) {
    const blockToken$1 = blockTokens[j2];
    if (blockToken$1.type !== "inline" || !state.md.linkify.pretest(blockToken$1.content)) continue;
...
function replacements(state) {
  ...
  for (let blkIdx = state.tokens.length - 1; blkIdx >= 0; blkIdx--) {
    ...
    const blkContent = blk.content || (Array.isArray(blk.children) ? blk.children.map((c2) => c2.type === "text" ? c2.content : "").join("") : "");
...
function process_inlines(tokens, state) {
  ...
  for (let i2 = 0; i2 < tokens.length; i2++) {
    ...
    for (j2 = stack.length - 1; j2 >= 0; j2--) if (stack[j2].level <= thisLevel) break;
    stack.length = j2 + 1;
    ...
    OUTER: while (pos < max) {
      ...
      else for (j2 = i2 + 1; j2 < tokens.length; j2++) {
        if (tokens[j2].type === "softbreak" || tokens[j2].type === "hardbreak") break;
        if (!tokens[j2].content) continue;
      ...
      if (canClose) for (j2 = stack.length - 1; j2 >= 0; j2--) {
        let item = stack[j2];
        if (stack[j2].level < thisLevel) break;
...
function smartquotes(state) {
  ...
  for (let blkIdx = state.tokens.length - 1; blkIdx >= 0; blkIdx--) {
    ...
    const inlineContent = typeof inlineToken$1.content === "string" ? inlineToken$1.content : (inlineToken$1.children || []).map((t2) => t2.content || "").join("");
...
function canUseParagraphTerminatorFastPath(state) {
  var _a2, _b;
  const ruler = (_b = (_a2 = state == null ? void 0 : state.md) == null ? void 0 : _a2.block) == null ? void 0 : _b.ruler;
...
var ParserCore = class {
  constructor() {
    ...
    __publicField(this, "cachedCoreRulesVersion", -1);
    __publicField(this, "cachedCoreRules", []);
    __publicField(this, "cachedCoreNamedRulesVersion", -1);
    __publicField(this, "cachedCoreNamedRules", []);
  ...
  process(state) {
    ...
    for (let i2 = 0; i2 < namedRules.length; i2++) {
      const startedAt = typeof performance !== "undefined" && typeof performance.now === "function" ? performance.now() : Date.now();
...
var PLAIN_CACHE_UNKNOWN = 0;
var PLAIN_CACHE_ENABLED = 1;
var PLAIN_CACHE_DISABLED = 2;
...
function unescapeAll(str) {
  ...
  return str.replace(UNESCAPE_ALL_RE, (match2, escaped, entity$1) => {
    ...
    const decoded = decodeHTML(match2);
    return decoded !== match2 ? decoded : match2;
  });
}
var INLINE_FALLBACK_RE = /[\n!#$%&*+\-:<=>@[\]\\^_`{}~]/;
var INLINE_SPECIAL_RE = /[\n!"#$%&*+\-:<=>@[\]\\^_`{}~]/;
var INLINE_QUOTE_RE = /"/g;
...
function splitIntoChunkRanges(src, opts, final = true) {
  ...
  for (let lineStart = 0; lineStart < src.length; ) {
    ...
    if (opts.fenceAware) {
      ...
      if (ch === "`" || ch === "~") {
        let q2 = p2;
        while (q2 < lineEnd$2 && src[q2] === ch) q2++;
        const runLen = q2 - p2;
        if (runLen >= 3) {
          if (!inFence) inFence = {
            marker: ch,
...
function hasUnsafeChunkBoundary(src, ranges, options = { rangesCoverWholeSource: true }) {
  ...
  for (let i2 = 0; i2 < limit; i2++) if (!endsAtBlankLine(src, ranges[i2].end)) return true;
  return false;
}
function endsAtBlankLine(src, end) {
  ...
  for (let i2 = lineStart + 1; i2 < end - 1; i2++) {
...
var UnboundedBuffer = class {
  constructor(md, opts = {}) {
    ...
    __publicField(this, "globalStateEnv", null);
    __publicField(this, "markedGlobalStateReason", null);
    this.md = md;
    this.options = {
      mode: "full",
      autoTune: true,
...
function parseStringUnbounded(md, src, env = {}, opts = {}) {
  ...
  if (currentGlobalStateReason) {
    ...
    if (opts.fallbackOnGlobalState === false) {
      const info = (_a2 = getParseDiagnostics(env)) == null ? void 0 : _a2.unbounded;
      if (info) {
...
function renderDefaultInlineToken(token, rules, inlineBreak, softbreak) {
  switch (token.type) {
    case "text":
      return rules.text === defaultRules.text ? token.content.length === 0 ? "" : escapeHtml$2(token.content) : null;
    case "text_special":
      return rules.text_special === defaultRules.text_special ? token.content.length === 0 ? "" : escapeHtml$2(token.content) : null;
    case "softbreak":
  ...
  render(tokens, options, env) {
    ...
    for (let i2 = 0; i2 < tokens.length; i2++) {
      ...
      if (type === "list_item_open" && (!token.attrs || token.attrs.length === 0) && i2 + 3 < tokens.length) {
        ...
        const inlineToken$1 = tokens[i2 + 2];
      ...
      if (i2 + 2 < tokens.length) {
        const inlineToken$1 = tokens[i2 + 1];
...
var EMPTY_TOKENS = [];
var GLOBAL_STATE_APPEND_SCAN_WINDOW = 4096;
function appendedHasBlockConstructs(s) {
  ...
  while (lineStart <= len) {
    ...
    if (p2 < lineEnd$2) {
      const ch = s.charCodeAt(p2);
      switch (ch) {
        case 35: {
          let q2 = p2;
          while (q2 < lineEnd$2 && s.charCodeAt(q2) === 35) q2++;
          const runLen = q2 - p2;
          if (runLen > 0 && runLen <= 6) {
            if (q2 < lineEnd$2) {
              const next = s.charCodeAt(q2);
              if (next === 32 || next === 9 || next === 13) return true;
            } else if (q2 === lineEnd$2 && hasLineBreak) return true;
          }
          break;
        }
        case 62: {
        ...
        case 126: {
          let q2 = p2;
          while (q2 < lineEnd$2 && s.charCodeAt(q2) === ch) q2++;
          if (q2 - p2 >= 3) return true;
          break;
        }
        default:
          if (ch >= 48 && ch <= 57) {
            let q2 = p2 + 1;
            while (q2 < lineEnd$2) {
              const d2 = s.charCodeAt(q2);
              if (d2 < 48 || d2 > 57) break;
              q2++;
            }
            if (q2 < lineEnd$2 && s.charCodeAt(q2) === 46) {
              const nextPos = q2 + 1;
              if (nextPos < lineEnd$2) {
                const next = s.charCodeAt(nextPos);
                if (next === 32 || next === 9 || next === 13) return true;
...
function attrsEqual(a2, b2) {
  if (!a2 && !b2) return true;
  if (!a2 || !b2 || a2.length !== b2.length) return false;
  for (let i2 = 0; i2 < a2.length; i2++) if (a2[i2][0] !== b2[i2][0] || a2[i2][1] !== b2[i2][1]) return false;
  return true;
}
function childrenEqual(a2, b2) {
  if (!a2 && !b2) return true;
  if (!a2 || !b2 || a2.length !== b2.length) return false;
  for (let i2 = 0; i2 < a2.length; i2++) if (!tokenEquals(a2[i2], b2[i2])) return false;
  return true;
}
function tokenEquals(x2, y2) {
  if (!x2 || !y2) return false;
  if (x2.type !== y2.type) return false;
  const xMap = x2.map;
  const yMap = y2.map;
  if (!!xMap !== !!yMap) return false;
  if (xMap && yMap && (xMap[0] !== yMap[0] || xMap[1] !== yMap[1])) return false;
  if (x2.tag !== y2.tag || x2.nesting !== y2.nesting) return false;
  if (x2.markup !== y2.markup || x2.info !== y2.info) return false;
  if (x2.block !== y2.block || x2.hidden !== y2.hidden) return false;
  if (!attrsEqual(x2.attrs, y2.attrs)) return false;
  if (!childrenEqual(x2.children, y2.children)) return false;
  return (x2.content || "") === (y2.content || "");
}
...
var StreamParser = class {
  ...
  parse(src, env, md) {
    ...
    beginParseDiagnostics(envProvided ?? (cached == null ? void 0 : cached.env));
    if (!cached || envProvided && envProvided !== cached.env) {
      ...
      const wantsChunking$1 = !!((_a2 = md.options) == null ? void 0 : _a2.streamChunkedFallback);
      ...
      const largeCachePolicy = ((_i = md.options) == null ? void 0 : _i.streamLargeCachePolicy) ?? "retain";
      const skipCacheChars = ((_j = md.options) == null ? void 0 : _j.streamSkipCacheAboveChars) ?? this.DEFAULT_SKIP_CACHE_CHARS;
      const skipCacheLines = ((_k = md.options) == null ? void 0 : _k.streamSkipCacheAboveLines) ?? this.DEFAULT_SKIP_CACHE_LINES;
      let srcLineCount;
      ...
      if (shouldSkipLargeCache) {
        ...
        setStrategyDiagnostics(workingEnv, {
          ...
          unbounded: !!((_l = getParseDiagnostics(workingEnv)) == null ? void 0 : _l.unbounded)
        });
      ...
      setStrategyDiagnostics(workingEnv, {
        ...
        unbounded: !!((_m = getParseDiagnostics(workingEnv)) == null ? void 0 : _m.unbounded)
      });
    ...
    if (nextGlobalStateReason) {
      ...
      setStrategyDiagnostics(fallbackEnv$1, {
        ...
        unbounded: !!((_n = getParseDiagnostics(fallbackEnv$1)) == null ? void 0 : _n.unbounded)
      });
      return nextTokens$1;
    }
    const threshold = ((_o = md.options) == null ? void 0 : _o.streamOptimizationMinSize) ?? this.MIN_SIZE_FOR_OPTIMIZATION;
    if (cached.src.length < threshold && src.length < threshold * 1.5 && !src.startsWith(cached.src)) {
      ...
      setStrategyDiagnostics(fallbackEnv$1, {
        ...
        unbounded: !!((_p = getParseDiagnostics(fallbackEnv$1)) == null ? void 0 : _p.unbounded)
      });
      return nextTokens$1;
    }
    ...
    if (appended && !this.shouldPreferTailReparseForAppend(cached)) {
      ...
      const ctxStrategy = ((_q = md.options) == null ? void 0 : _q.streamContextParseStrategy) ?? "chars";
      const CONTEXT_PARSE_MIN_CHARS = ((_r = md.options) == null ? void 0 : _r.streamContextParseMinChars) ?? 200;
      const CONTEXT_PARSE_MIN_LINES = ((_s = md.options) == null ? void 0 : _s.streamContextParseMinLines) ?? 2;
      ...
      if (appendedState.tokens.length > appendedTokenStart) {
        ...
        const maxCheck = Math.min(cachedTail.length, a2.length - appendedTokenStart);
        let dup = 0;
        for (let n2 = maxCheck; n2 > 0; n2--) {
          let ok = true;
          for (let i2 = 0; i2 < n2; i2++) {
            const tailToken = cachedTail[cachedTail.length - n2 + i2];
            const prefToken = a2[appendedTokenStart + i2];
        ...
        if (a2.length > appendedTokenStart) this.appendTokens(cached.tokens, a2, appendedTokenStart);
      }
      cached.src = src;
    ...
    const wantsChunking = !!((_t = md.options) == null ? void 0 : _t.streamChunkedFallback);
    ...
    const chunkAdaptive = ((_u = md.options) == null ? void 0 : _u.streamChunkAdaptive) !== false;
    const targetChunks = ((_v = md.options) == null ? void 0 : _v.streamChunkTargetChunks) ?? 8;
    const chunkSizeCharsCfg = (_w = md.options) == null ? void 0 : _w.streamChunkSizeChars;
    const chunkSizeLinesCfg = (_x = md.options) == null ? void 0 : _x.streamChunkSizeLines;
    const chunkMaxChunksCfg = (_y = md.options) == null ? void 0 : _y.streamChunkMaxChunks;
    const explicitChunkConfig = !!md.__explicitStreamChunkConfig;
    const auto = ((_z = md.options) == null ? void 0 : _z.autoTuneChunks) !== false;
    const chunkFenceAware = ((_A = md.options) == null ? void 0 : _A.streamChunkFenceAware) ?? true;
    ...
    setStrategyDiagnostics(fallbackEnv, {
      ...
      unbounded: !!((_B = getParseDiagnostics(fallbackEnv)) == null ? void 0 : _B.unbounded)
    });
    return nextTokens;
  }
  recordChunkedParseResult(env, chunkReason) {
    var _a2, _b;
    const chunkInfo = (_a2 = getParseDiagnostics(env)) == null ? void 0 : _a2.chunk;
    const fallbackReason = (chunkInfo == null ? void 0 : chunkInfo.fallback) ? String(chunkInfo.fallbackReason || "global-state") : null;
    this.stats.total += 1;
    if (fallbackReason) {
      this.stats.fullParses += 1;
      this.stats.lastMode = "full";
      setStrategyDiagnostics(env, {
        ...
        unbounded: !!((_b = getParseDiagnostics(env)) == null ? void 0 : _b.unbounded)
      });
      return;
    }
  ...
  endsInsideOpenFence(text$1) {
    ...
    while (lineStart <= len) {
      ...
      let p2 = lineStart;
      while (p2 < lineEnd$2) {
        const c2 = chunk.charCodeAt(p2);
        if (c2 === 32 || c2 === 9) p2++;
        else break;
      }
      if (p2 < lineEnd$2) {
        const ch = chunk.charCodeAt(p2);
        if (ch === 96 || ch === 126) {
          let q2 = p2;
          while (q2 < lineEnd$2 && chunk.charCodeAt(q2) === ch) q2++;
          const runLen = q2 - p2;
          if (runLen >= 3) {
            if (!inFence) inFence = {
              marker: ch,
  ...
  appendTokens(target, source, start = 0, end = source.length) {
    for (let i2 = start; i2 < end; i2++) target.push(source[i2]);
  }
  ...
  getLastSegment(tokens, src, tokenStart = 0, tokenEnd = tokens.length, knownSrcOffset, knownLineStart) {
    ...
    let lineEnd$2 = -1;
    let depth = 0;
    for (let i2 = tokenEnd - 1; i2 >= tokenStart; i2--) {
      const token = tokens[i2];
      if (token.map) {
      ...
      if (token.nesting > 0) {
        ...
        if (token.level === 0 && depth <= 0) {
          const resolvedStart = Number.isFinite(lineStart) ? lineStart : ((_a2 = token.map) == null ? void 0 : _a2[0]) ?? 0;
          const resolvedEnd = lineEnd$2 >= resolvedStart ? lineEnd$2 : ((_b = token.map) == null ? void 0 : _b[1]) ?? resolvedStart;
          return {
            tokenStart: i2,
            tokenEnd,
      ...
      if (token.level === 0 && depth === 0) {
        ...
        const resolvedEnd = lineEnd$2 >= resolvedStart ? lineEnd$2 : ((_d = token.map) == null ? void 0 : _d[1]) ?? resolvedStart;
        return {
          tokenStart: i2,
          tokenEnd,
          lineStart: resolvedStart,
  ...
  canDirectlyParseAppend(cache) {
    ...
    switch ((_a2 = cache.tokens[lastSegment.tokenStart]) == null ? void 0 : _a2.type) {
      case "paragraph_open":
      case "heading_open":
      case "fence":
      case "code_block":
  ...
  tryListTailAppendMerge(src, cached, env, md, lastSegment, appended, listOpen) {
    ...
    if (existingMode === "loose" || appendedMode === "loose" || this.endsWithBlankLine(cached.src) || (((_b = (_a2 = parsed[0]) == null ? void 0 : _a2.map) == null ? void 0 : _b[0]) ?? 0) > 0) {
  ...
  tryTableTailAppendMerge(src, cached, env, md, lastSegment, appended, tableOpen) {
    ...
    if ((((_b = (_a2 = parsed[0]) == null ? void 0 : _a2.map) == null ? void 0 : _b[1]) ?? -1) !== this.getDocLineCount(syntheticSrc)) return null;
  ...
  getTableBodySection(tokens, start, end, tableLevel) {
    var _a2;
    if (start < 0 || start >= end || ((_a2 = tokens[start]) == null ? void 0 : _a2.type) !== "table_open") return null;
...
function setFullChunkStrategyDiagnostics(env, reason) {
  var _a2;
  const chunkInfo = (_a2 = getParseDiagnostics(env)) == null ? void 0 : _a2.chunk;
  if (chunkInfo == null ? void 0 : chunkInfo.fallback) {
    setStrategyDiagnostics(env, {
      area: "parse",
      path: "plain",
...
function markdownIt(presetName, options) {
  ...
  const md = {
    ...
    configure(presets) {
      var _a3, _b2, _c2, _d2;
      const p2 = typeof presets === "string" ? config[presets] : presets;
      if (!p2) throw new Error("Wrong `markdown-it` preset, can't be empty");
      if (p2.options) this.set(p2.options);
      if (p2.components) {
        const c2 = p2.components;
        if ((_a3 = c2.core) == null ? void 0 : _a3.rules) this.core.ruler.enableOnly(c2.core.rules);
        if ((_b2 = c2.block) == null ? void 0 : _b2.rules) this.block.ruler.enableOnly(c2.block.rules);
        if ((_c2 = c2.inline) == null ? void 0 : _c2.rules) this.inline.ruler.enableOnly(c2.inline.rules);
        if ((_d2 = c2.inline2) == null ? void 0 : _d2.rules) this.inline.ruler2.enableOnly(c2.inline2.rules);
      }
      return this;
    },
    enable(list$1, ignoreInvalid) {
      ...
      for (const m2 of managers) {
        if (!m2) continue;
        const enabled = m2.enable(names, true);
        for (let i2 = 0; i2 < enabled.length; i2++) found.add(enabled[i2]);
      }
      if (!ignoreInvalid) {
    ...
    disable(list$1, ignoreInvalid) {
      ...
      for (const m2 of managers) {
        if (!m2) continue;
        const disabled = m2.disable(names, true);
        for (let i2 = 0; i2 < disabled.length; i2++) found.add(disabled[i2]);
      }
      if (!ignoreInvalid) {
        const missed = names.filter((name) => !found.has(name));
        if (missed.length) throw new Error(`Rules manager: invalid rule name ${missed.join(", ")}`);
  ...
  if (preset == null ? void 0 : preset.components) {
    const c2 = preset.components;
    if ((_a2 = c2.core) == null ? void 0 : _a2.rules) md.core.ruler.enableOnly(c2.core.rules);
    if ((_b = c2.block) == null ? void 0 : _b.rules) md.block.ruler.enableOnly(c2.block.rules);
    if ((_c = c2.inline) == null ? void 0 : _c.rules) md.inline.ruler.enableOnly(c2.inline.rules);
    if ((_d = c2.inline2) == null ? void 0 : _d.rules) md.inline.ruler2.enableOnly(c2.inline2.rules);
  }
...
function decodeHtmlUrlEntities(value) {
  return value.replace(/&(?:#(\d+)|#x([0-9a-f]+)|([a-z][a-z0-9]+));?/gi, (match2, decimal, hex, named) => {
    ...
    return HTML_URL_ENTITY_MAP[String(named ?? "").toLowerCase()] ?? match2;
  });
}
var HREF_URL_PROTOCOLS = /* @__PURE__ */ new Set([
  "http",
  "https",
  "mailto",
  "tel"
]);
var BLOCKED_HREF_URL_PROTOCOLS = /* @__PURE__ */ new Set([
  "javascript",
  "vbscript",
  "data",
  "file",
...
var RESOURCE_URL_PROTOCOLS = /* @__PURE__ */ new Set(["http", "https"]);
function getUrlScheme(normalized) {
  var _a2, _b;
  return ((_b = (_a2 = normalized.match(/^([a-z][a-z0-9+.-]*):/i)) == null ? void 0 : _a2[1]) == null ? void 0 : _b.toLowerCase()) ?? "";
}
var PLAIN_SAFE_HTTP_URL_RE = /^https?:\/\//i;
...
function applyContainers(md) {
  ...
  md.block.ruler.before("fence", "vmr_container_fallback", (state, startLine, endLine, silent) => {
    ...
    for (let i2 = startLine + 1; i2 < nextLine; i2++) {
      const sPos = s.bMarks[i2] + s.tShift[i2];
      const ePos = s.eMarks[i2];
...
function getHtmlInlineTagName(content) {
  var _a2;
  return (((_a2 = content.match(TAG_NAME_AT_START_RE)) == null ? void 0 : _a2[1]) ?? "").toLowerCase();
}
...
function attachCustomHtmlSourceMeta(tokens, customTagSet) {
  ...
  const closeTopFrameWithRaw = (raw) => {
    for (let i2 = 0; i2 < stack.length; i2++) {
      stack[i2].raw += raw;
      if (i2 < stack.length - 1) stack[i2].inner += raw;
    }
  ...
  const getTopFrameClosePrefix = (raw) => {
    var _a2, _b;
    const tag = (_a2 = stack[stack.length - 1]) == null ? void 0 : _a2.tag;
    if (!tag) return null;
    const closePrefixRe = new RegExp(String.raw`^\s*<\s*\/\s*${escapeTagForRegExp(tag)}\s*>`, "i");
    return ((_b = raw.match(closePrefixRe)) == null ? void 0 : _b[0]) ?? null;
  };
...
function buildCommonHtmlTagSet(extraTags) {
  ...
  if (extraTags && Array.isArray(extraTags)) for (const t2 of extraTags) {
    ...
    if (!raw) continue;
    const m2 = raw.match(/^[<\s/]*([A-Z][\w-]*)/i);
    if (!m2) continue;
    set2.add(m2[1].toLowerCase());
  }
  return set2;
}
...
function findFirstIncompleteTag(content, tagSet) {
  let first = null;
  for (const m2 of content.matchAll(OPEN_TAG_RE)) {
    const idx = m2.index ?? -1;
    if (idx < 0) continue;
  ...
  for (const m2 of content.matchAll(CLOSE_TAG_RE)) {
    const idx = m2.index ?? -1;
    if (idx < 0) continue;
    const tag = (m2[1] ?? "").toLowerCase();
    if (!isCommonHtmlTagOrPrefix(tag, tagSet)) continue;
...
var BASE_AUTO_CLOSE_INLINE_TAGS = [
  "a",
  "span",
  "strong",
  "em",
...
function applyFixHtmlInlineTokens(md, options = {}) {
  ...
  const getRuleContext = (state) => {
    ...
    const envTags = Array.isArray((_a3 = s.env) == null ? void 0 : _a3.__markstreamCustomHtmlTags) ? s.env.__markstreamCustomHtmlTags : [];
    for (const t2 of envTags) {
      const name = normalizeCustomHtmlTagName(String(t2 ?? ""));
  ...
  const getTrailingContentTextMode = (tokens, index, fallback) => {
    var _a3, _b;
    return ((_a3 = tokens[index - 1]) == null ? void 0 : _a3.type) === "paragraph_open" && ((_b = tokens[index + 1]) == null ? void 0 : _b.type) === "paragraph_close" ? "inline" : fallback;
  };
...
${chunk}`;
            ...
            if (closeRange) {
              ...
              toks.splice(i2, 1);
              tagStack.pop();
              const replacement = appendTrailingInlineContent(openToken, after) ? [] : createTrailingContentTokens(after, getTrailingContentTextMode(toks, i2, "paragraph"));
              if (replacement.length) toks.splice(i2, 0, ...replacement);
              i2--;
              continue;
            }
...
${closeContent}`;
            ...
            stack.pop();
            const replacement = appendTrailingInlineContent(openTok, afterContent) ? [] : createTrailingContentTokens(afterContent, getTrailingContentTextMode(toks, i2, "paragraph"));
            if (replacement.length) toks.splice(i2, 1, ...replacement);
            else {
              toks.splice(i2, 1);
              i2--;
            }
          ...
          if (closeChildIndex !== -1) {
            ...
            const beforeText = beforeChildren.map((c2) => String((c2 == null ? void 0 : c2.content) ?? (c2 == null ? void 0 : c2.raw) ?? "")).join("");
            openTok.content = `${String(openTok.content ?? "")}
${beforeText}`;
            if (Array.isArray(openTok.children)) openTok.children.push(...beforeChildren);
            if (afterChildren.length) {
              const afterText = afterChildren.map((c2) => String(c2.content ?? c2.raw ?? "")).join("");
              if (afterText.trim()) {
                const trimmed = afterText.replace(/^\s+/, "");
                if (appendTrailingInlineContent(openTok, afterText)) {
                  toks.splice(i2, 1);
                  i2--;
                } else if (trimmed.startsWith("<")) toks.splice(i2, 1, {
                  type: "html_block",
                  content: trimmed
                });
                else {
                  const replacement = createTrailingContentTokens(afterText, getTrailingContentTextMode(toks, i2, "paragraph"));
                  toks.splice(i2, 1, ...replacement);
                }
              } else {
                toks.splice(i2, 1);
                i2--;
    ...
    for (let i2 = 0; i2 < toks.length; i2++) {
      ...
      if (t2.type === "html_block") {
        ...
        if (customTagSet.has(tag)) {
          ...
          if (endTagIndex$1 !== -1) {
            ...
            if (replacement.length) toks.splice(i2 + 1, 0, ...replacement);
          } else t2.children = [{
            type: tag,
            content: "",
            raw: raw$2,
      ...
      if (t2.children.length === 2 && t2.children[0].type === "html_inline") {
        ...
        if (autoCloseInlineTagSet.has(tag)) {
          ...
          loading: true,
          tag,
          content: String(((_h = t2.children[0]) == null ? void 0 : _h.content) ?? "") + String(((_i = t2.children[1]) == null ? void 0 : _i.content) ?? "")
        }];
        continue;
        ...
        t2.children = [{
          ...
          loading: false,
          tag,
          content: t2.children.map((ct) => ct.content).join("")
        }];
        continue;
      }
      if (!((_l = t2.content) == null ? void 0 : _l.startsWith("<")) || ((_m = t2.children) == null ? void 0 : _m.length) !== 1) continue;
      ...
      const strictTagName = ((_o = (_n = String(onlyChild.content ?? raw).match(STRICT_OPEN_TAG_NAME_AT_START_RE)) == null ? void 0 : _n[1]) == null ? void 0 : _o.toLowerCase()) ?? "";
...
var FILENAMEISH_EXTENSION_RE = /\.([a-z0-9]{1,15})$/i;
...
var MARKET_TICKER_SYMBOL_RE = /^(?:[A-Z]{1,6}|\d{1,8})$/u;
var MARKET_TICKER_CONTEXT_SYMBOL_RE = /^(?=.{1,12}$)[A-Z0-9]+(?:[-.][A-Z0-9]+)*$/iu;
var EXPLICIT_FILENAME_CONTEXT_RE = /文件名\s*[:：]?|附件\s*[:：]?|路径\s*[:：]?|路徑\s*[:：]?|文件列表\s*[:：]?|文档列表\s*[:：]?|文檔列表\s*[:：]?|\bfile\s*names?\b\s*[:：]?|\battachments?\b\s*[:：]?|\bpaths?\b\s*[:：]?|\bfile\s+lists?\b\s*[:：]?|\bdocument\s+lists?\b\s*[:：]?/iu;
var FILENAME_CONTEXT_RE = /文件名\s*[:：]?|文件\s*[:：]?|附件\s*[:：]?|档案\s*[:：]?|檔案\s*[:：]?|文档\s*[:：]?|文檔\s*[:：]?|资料\s*[:：]?|資料\s*[:：]?|路径\s*[:：]?|路徑\s*[:：]?|\bfile\s*name\b\s*[:：]?|\battachments?\b\s*[:：]?|\bfiles?\b\s*[:：]?|\bdocuments?\b\s*[:：]?|\bdocs?\b\s*[:：]?|\bpaths?\b\s*[:：]?/iu;
var MARKET_TICKER_CONTEXT_RE = /股票代码|股票代碼|证券代码|證券代碼|(?:代码|代碼|交易所|后缀|後綴|市场|市場)(?=$|[\s:：/|,，、()（）])|\btickers?\b|\bsymbols?\b|\bexchanges?\b/iu;
var LINKIFY_DEMOTION_CONTEXT_CACHE_LIMIT = 2e3;
var LINKIFY_DEMOTION_CONTEXT_CACHE_MAX_TEXT_LENGTH = 512;
var EMPTY_LINKIFY_DEMOTION_CONTEXT = {};
...
var MARKET_TICKER_SUFFIXES = /* @__PURE__ */ new Set([
  "as",
  "bj",
  "de",
  "hk",
...
var MARKET_TICKER_CONTEXT_SUFFIXES = /* @__PURE__ */ new Set([
...
var EXPLICIT_FILENAME_CONTEXT_ONLY_EXTENSIONS = /* @__PURE__ */ new Set([
  "com",
  "dev",
  "io",
  "page",
...
var FILENAME_CONTEXT_ONLY_EXTENSIONS = /* @__PURE__ */ new Set([
  "app",
  "apk",
  "dmg",
  "exe",
...
var linkifyDemotionContextCache = /* @__PURE__ */ new Map();
...
function hasLinkifyDemotionContext(context) {
  return (context == null ? void 0 : context.filename) === true || (context == null ? void 0 : context.explicitFilename) === true || (context == null ? void 0 : context.marketTicker) === true;
}
function mergeLinkifyDemotionContext(left, right) {
  const merged = {
    filename: (left == null ? void 0 : left.filename) || (right == null ? void 0 : right.filename),
    explicitFilename: (left == null ? void 0 : left.explicitFilename) || (right == null ? void 0 : right.explicitFilename),
    marketTicker: (left == null ? void 0 : left.marketTicker) || (right == null ? void 0 : right.marketTicker)
  };
...
function withLinkifyDemotionContext(options, context) {
  ...
  const inheritedContext = options == null ? void 0 : options.__linkifyDemotionContext;
  return {
    ...options,
    __linkifyDemotionContext: {
      filename: (inheritedContext == null ? void 0 : inheritedContext.filename) || (context == null ? void 0 : context.filename),
      explicitFilename: (inheritedContext == null ? void 0 : inheritedContext.explicitFilename) || (context == null ? void 0 : context.explicitFilename),
      marketTicker: (inheritedContext == null ? void 0 : inheritedContext.marketTicker) || (context == null ? void 0 : context.marketTicker)
    }
  };
}
...
function inferContinuationLinkifyContext(raw, inherited) {
  ...
  const continuation = {};
  if ((inherited == null ? void 0 : inherited.filename) && parts.every((part) => shouldDemoteFilenameLikeLinkify(part, {
    filename: true,
    explicitFilename: inherited.explicitFilename
  }))) continuation.filename = true;
  if ((inherited == null ? void 0 : inherited.explicitFilename) && continuation.filename) continuation.explicitFilename = true;
  if ((inherited == null ? void 0 : inherited.marketTicker) && parts.every((part) => shouldDemoteFilenameLikeLinkify(part, { marketTicker: true }))) continuation.marketTicker = true;
...
function isPlausibleBareDomain(text$1) {
  ...
  const tld = ((_a2 = labels[labels.length - 1]) == null ? void 0 : _a2.toLowerCase()) ?? "";
...
function getHrefFromLinkOpen(token) {
  var _a2, _b;
  const href = (_b = (_a2 = token.attrs) == null ? void 0 : _a2.find((attr) => (attr == null ? void 0 : attr[0]) === "href")) == null ? void 0 : _b[1];
  ...
  md.core.ruler.after("inline", "fix_link_tokens", (state) => {
    ...
    for (let i2 = 0; i2 < toks.length; i2++) {
      ...
      if (t2 && t2.type === "inline" && Array.isArray(t2.children)) try {
        t2.children = fixLinkToken(t2.children, typeof t2.content === "string" ? t2.content : void 0);
      } catch (e2) {
...
function fixLinkToken(tokens, raw) {
  ...
  for (let index = 0; index < tokens.length; index++) {
    ...
    if (token.type === "link_open") {
      let closeIndex = -1;
      for (let candidate = index + 1; candidate < tokens.length; candidate++) if (((_a2 = tokens[candidate]) == null ? void 0 : _a2.type) === "link_close") {
  ...
  for (let i2 = 0; i2 <= tokens.length - 1; i2++) {
    ...
    if (curToken.type === "link_open" && (curToken.markup === "linkify" || curToken.markup === "autolink")) {
      let closeIdx = -1;
      for (let j2 = i2 + 1; j2 < tokens.length; j2++) if (((_b = tokens[j2]) == null ? void 0 : _b.type) === "link_close") {
        closeIdx = j2;
        break;
      }
      if (closeIdx !== -1) {
        ...
        if (curToken.markup === "linkify" && (linkText == null ? void 0 : linkText.includes("）")) && (fullwidthParenDepthAtLink.get(curToken) ?? 0) > 0) {
        ...
        for (let j2 = i2 + 1; j2 < closeIdx; j2++) {
          const t2 = tokens[j2];
          if ((t2 == null ? void 0 : t2.type) !== "text" || typeof t2.content !== "string") continue;
          if (remainingStopAt >= t2.content.length) {
            remainingStopAt -= t2.content.length;
            continue;
          }
          if (remainingStopAt < 0) break;
          const stopChar = t2.content[remainingStopAt];
          const before = t2.content.slice(0, remainingStopAt);
          let tail = t2.content.slice(remainingStopAt);
          for (let k2 = j2 + 1; k2 < closeIdx; k2++) {
            const tk = tokens[k2];
            if ((tk == null ? void 0 : tk.type) === "text" && typeof tk.content === "string") tail += tk.content;
          }
          t2.content = before;
          t2.raw = before;
          const removeCount = closeIdx - (j2 + 1);
          if (removeCount > 0) {
            tokens.splice(j2 + 1, removeCount);
            closeIdx = j2 + 1;
          }
          let newHref = href;
    ...
    if ((curToken == null ? void 0 : curToken.type) === "em_open" && ((_c = tokens[i2 - 1]) == null ? void 0 : _c.type) === "text" && ((_d = tokens[i2 - 1].content) == null ? void 0 : _d.endsWith("*"))) {
      const beforeText = ((_e = tokens[i2 - 1].content) == null ? void 0 : _e.replace(/(\*+)$/, "")) || "";
      tokens[i2 - 1].content = beforeText;
      curToken.type = "strong_open";
      curToken.tag = "strong";
      curToken.markup = "**";
      for (let j2 = i2 + 1; j2 < tokens.length; j2++) if (((_f = tokens[j2]) == null ? void 0 : _f.type) === "em_close") {
        tokens[j2].type = "strong_close";
        tokens[j2].tag = "strong";
        tokens[j2].markup = "**";
        break;
      }
    } else if ((curToken == null ? void 0 : curToken.type) === "text" && ((_g = curToken.content) == null ? void 0 : _g.endsWith("(")) && ((_h = tokens[i2 + 1]) == null ? void 0 : _h.type) === "link_open") {
      ...
      if (match2) {
        ...
        if (emphasisMatch) {
          ...
          let href = ((_i = tokens[i2 + 2]) == null ? void 0 : _i.content) || "";
          if (((_j = tokens[i2 + 4]) == null ? void 0 : _j.type) === "text" && !((_k = tokens[i2 + 4].content) == null ? void 0 : _k.startsWith(")"))) {
            href += ((_l = tokens[i2 + 4]) == null ? void 0 : _l.content) || "";
            tokens[i2 + 4].content = "";
          }
          replacerTokens.push(createLinkToken(text$1, href, !((_n = (_m = tokens[i2 + 4]) == null ? void 0 : _m.content) == null ? void 0 : _n.startsWith(")"))));
          pushEmClose(replacerTokens, type);
          if (((_o = tokens[i2 + 4]) == null ? void 0 : _o.type) === "text") {
            const afterText = (_p = tokens[i2 + 4].content) == null ? void 0 : _p.replace(/^\)\**/, "");
          ...
          if (emphasisMatch$1) {
            ...
            let href$1 = ((_q = tokens[i2 + 2]) == null ? void 0 : _q.content) || "";
            if (((_r = tokens[i2 + 4]) == null ? void 0 : _r.type) === "text" && !((_s = tokens[i2 + 4].content) == null ? void 0 : _s.startsWith(")"))) {
              href$1 += ((_t = tokens[i2 + 4]) == null ? void 0 : _t.content) || "";
              tokens[i2 + 4].content = "";
            }
            pushEmOpen(replacerTokens, type);
            replacerTokens.push(createLinkToken(text$1, href$1, !((_v = (_u = tokens[i2 + 4]) == null ? void 0 : _u.content) == null ? void 0 : _v.startsWith(")"))));
            pushEmClose(replacerTokens, type);
            if (((_w = tokens[i2 + 4]) == null ? void 0 : _w.type) === "text") {
              const afterText = (_x = tokens[i2 + 4].content) == null ? void 0 : _x.replace(/^\)/, "");
              if (afterText) replacerTokens.push(textToken(afterText));
              tokens.splice(i2, 5, ...replacerTokens);
            } else tokens.splice(i2, 4, ...replacerTokens);
          ...
          let href = ((_y = tokens[i2 + 2]) == null ? void 0 : _y.content) || "";
          if (((_z = tokens[i2 + 4]) == null ? void 0 : _z.type) === "text" && !((_A = tokens[i2 + 4].content) == null ? void 0 : _A.startsWith(")"))) {
            href += ((_B = tokens[i2 + 4]) == null ? void 0 : _B.content) || "";
            tokens[i2 + 4].content = "";
          }
          replacerTokens.push(createLinkToken(text$1, href, !((_D = (_C = tokens[i2 + 4]) == null ? void 0 : _C.content) == null ? void 0 : _D.startsWith(")"))));
          if (((_E = tokens[i2 + 4]) == null ? void 0 : _E.type) === "text") {
            const afterText = (_F = tokens[i2 + 4].content) == null ? void 0 : _F.replace(/^\)/, "");
            if (afterText) replacerTokens.push(textToken(afterText));
            tokens.splice(i2, 5, ...replacerTokens);
          } else tokens.splice(i2, 4, ...replacerTokens);
        }
    ...
    } else if (curToken.type === "link_open" && curToken.markup === "linkify" && ((_G = tokens[i2 - 1]) == null ? void 0 : _G.type) === "text" && ((_H = tokens[i2 - 1].content) == null ? void 0 : _H.endsWith("("))) {
      if (((_I = tokens[i2 - 2]) == null ? void 0 : _I.type) === "link_close") {
        const replacerTokens = [];
        const text$1 = tokens[i2 - 3].content || "";
        let href = ((_K = (_J = curToken.attrs) == null ? void 0 : _J.find((attr) => attr[0] === "href")) == null ? void 0 : _K[1]) || "";
        if (((_L = tokens[i2 + 3]) == null ? void 0 : _L.type) === "text") {
          const m2 = (((_M = tokens[i2 + 3]) == null ? void 0 : _M.content) ?? "").indexOf(")");
          const loading = m2 === -1;
          if (m2 === -1) {
            href += ((_O = (_N = tokens[i2 + 3]) == null ? void 0 : _N.content) == null ? void 0 : _O.slice(0, m2)) || "";
            tokens[i2 + 3].content = "";
          }
          replacerTokens.push(createLinkToken(text$1, href, loading));
          const afterText = (_P = tokens[i2 + 3].content) == null ? void 0 : _P.replace(/^\)\**/, "");
          if (afterText) replacerTokens.push(textToken(afterText));
        ...
        continue;
      } else if (tokens[i2 - 1].content === "](" && ((_Q = tokens[i2 - 3]) == null ? void 0 : _Q.type) === "text" && ((_R = tokens[i2 - 3].content) == null ? void 0 : _R.endsWith(")"))) if (((_S = tokens[i2 - 2]) == null ? void 0 : _S.type) === "strong_open") {
        ...
        tokens[i2 - 1].content = "";
      } else if (((_U = tokens[i2 - 2]) == null ? void 0 : _U.type) === "em_open") {
        const [beforeText, linText] = ((_V = tokens[i2 - 3].content) == null ? void 0 : _V.split("[*")) || [];
        tokens[i2 + 1].content = linText || "";
        tokens[i2 - 3].content = beforeText || "";
        tokens[i2 - 1].content = "";
      } else {
        const [beforeText, linText] = ((_W = tokens[i2 - 3].content) == null ? void 0 : _W.split("[")) || [];
        tokens[i2 + 1].content = linText || "";
        tokens[i2 - 3].content = beforeText || "";
        tokens[i2 - 1].content = "";
      }
    }
    if (curToken.type === "link_close" && curToken.nesting === -1 && ((_X = tokens[i2 - 2]) == null ? void 0 : _X.type) === "link_open" && ((_Y = tokens[i2 + 1]) == null ? void 0 : _Y.type) === "text" && ((_Z = tokens[i2 - 1]) == null ? void 0 : _Z.type) === "text") {
      const text$1 = tokens[i2 - 1].content || "";
      const attrs = tokens[i2 - 2].attrs || [];
      const href = ((__ = attrs.find((a2) => a2[0] === "href")) == null ? void 0 : __[1]) || "";
      const title = ((_$ = attrs.find((a2) => a2[0] === "title")) == null ? void 0 : _$[1]) || "";
      let count = 3;
      let deleteCount = 2;
      const emphasisMatch = (((_aa = tokens[i2 - 3]) == null ? void 0 : _aa.content) || "").match(/^(\*+)$/);
      const replacerTokens = [];
      if (emphasisMatch) {
        deleteCount += 1;
        const type = emphasisMatch[1].length;
      ...
      if (curToken.markup !== "linkify" && tokens[i2 + 1].type === "text" && ((_ca = (_ba = tokens[i2 + 1]) == null ? void 0 : _ba.content) == null ? void 0 : _ca.startsWith("]("))) {
        count += 1;
      ...
      i2 -= replacerTokens.length + 1;
      continue;
    } else if (((_da = curToken.content) == null ? void 0 : _da.startsWith("](")) && ((_ea = tokens[i2 - 1].markup) == null ? void 0 : _ea.includes("*")) && ((_fa = tokens[i2 - 4]) == null ? void 0 : _fa.type) === "text" && ((_ga = tokens[i2 - 4].content) == null ? void 0 : _ga.endsWith("["))) {
      ...
      let loading = true;
      if (((_ha = tokens[i2 + 1]) == null ? void 0 : _ha.type) === "text") {
        const m2 = (((_ia = tokens[i2 + 1]) == null ? void 0 : _ia.content) ?? "").indexOf(")");
        loading = m2 === -1;
        if (m2 === -1) {
          href += ((_ka = (_ja = tokens[i2 + 1]) == null ? void 0 : _ja.content) == null ? void 0 : _ka.slice(0, m2)) || "";
          tokens[i2 + 1].content = "";
        }
      }
      replacerTokens.push(createLinkToken(text$1, href, loading));
      pushEmClose(replacerTokens, type);
      if (((_la = tokens[i2 + 1]) == null ? void 0 : _la.type) === "text") {
        const afterText = (_ma = tokens[i2 + 1].content) == null ? void 0 : _ma.replace(/^\)\**/, "");
        if (afterText) replacerTokens.push(textToken(afterText));
        tokens.splice(i2 - 4, 8, ...replacerTokens);
      } else if (((_na = tokens[i2 + 1]) == null ? void 0 : _na.type) === "link_open") tokens.splice(i2 - 4, 10, ...replacerTokens);
      else tokens.splice(i2 - 4, 7, ...replacerTokens);
      i2 -= replacerTokens.length + 1;
      continue;
    } else if (((_oa = curToken.content) == null ? void 0 : _oa.startsWith("](")) && tokens[i2 - 1].type === "strong_close" && ((_pa = tokens[i2 - 4]) == null ? void 0 : _pa.type) === "text" && ((_ra = (_qa = tokens[i2 - 4]) == null ? void 0 : _qa.content) == null ? void 0 : _ra.includes("**["))) {
      const replacerTokens = [];
      ...
      let href = curToken.content.slice(2);
      let loading = true;
      if (((_sa = tokens[i2 + 1]) == null ? void 0 : _sa.type) === "text") {
        const m2 = (((_ta = tokens[i2 + 1]) == null ? void 0 : _ta.content) ?? "").indexOf(")");
        loading = m2 === -1;
        if (m2 === -1) {
          href += ((_va = (_ua = tokens[i2 + 1]) == null ? void 0 : _ua.content) == null ? void 0 : _va.slice(0, m2)) || "";
          tokens[i2 + 1].content = "";
        }
      }
      replacerTokens.push(createLinkToken(text$1, href, loading));
      pushEmClose(replacerTokens, 2);
      if (((_wa = tokens[i2 + 1]) == null ? void 0 : _wa.type) === "text") {
        const afterText = (_xa = tokens[i2 + 1].content) == null ? void 0 : _xa.replace(/^\)\**/, "");
        if (afterText) replacerTokens.push(textToken(afterText));
        tokens.splice(i2 - 4, 8, ...replacerTokens);
      } else if (((_ya = tokens[i2 + 1]) == null ? void 0 : _ya.type) === "link_open") tokens.splice(i2 - 4, 10, ...replacerTokens);
      else tokens.splice(i2 - 4, 7, ...replacerTokens);
      i2 -= replacerTokens.length + 1;
      continue;
    } else if (curToken.type === "strong_close" && ((_za = tokens[i2 + 1]) == null ? void 0 : _za.type) === "text" && ((_Aa = tokens[i2 + 1].content) == null ? void 0 : _Aa.includes("](")) && tokens[i2 - 1].type === "text" && /\[.*$/.test(tokens[i2 - 1].content || "")) {
      const replacerTokens = [];
      const [beforeText, afterText] = ((_Ba = tokens[i2 - 1].content) == null ? void 0 : _Ba.split("[")) || ["", ""];
      if (beforeText) replacerTokens.push(textToken(beforeText));
      pushEmOpen(replacerTokens, 2);
      let [text$1, href] = tokens[i2 + 1].content.split("](");
      text$1 = afterText + text$1;
      let deleteCount = 4;
      if (((_Ca = tokens[i2 + 2]) == null ? void 0 : _Ca.type) === "link_open") {
        const _href = (_Ea = (_Da = tokens[i2 + 2].attrs) == null ? void 0 : _Da.find((a2) => a2[0] === "href")) == null ? void 0 : _Ea[1];
        if (((_Fa = tokens[i2 + 5]) == null ? void 0 : _Fa.type) === "text" && tokens[i2 + 5].content === ".") {
      ...
      if (((_Ga = tokens[i2 + 2]) == null ? void 0 : _Ga.type) === "text") {
        const m2 = (((_Ha = tokens[i2 + 2]) == null ? void 0 : _Ha.content) ?? "").indexOf(")");
        loading = m2 === -1;
        if (m2 === -1) {
          href += ((_Ja = (_Ia = tokens[i2 + 2]) == null ? void 0 : _Ia.content) == null ? void 0 : _Ja.slice(0, m2)) || "";
          tokens[i2 + 2].content = "";
        }
      }
      replacerTokens.push(createLinkToken(text$1, href, loading));
    ...
    if (curToken.type === "text" && /\*+\[[^\]]*$/.test(curToken.content || "") && ((_Ka = tokens[i2 + 1]) == null ? void 0 : _Ka.type) === "strong_open" && ((_La = tokens[i2 + 2]) == null ? void 0 : _La.type) === "text" && tokens[i2 + 2].content === "](" && ((_Ma = tokens[i2 + 3]) == null ? void 0 : _Ma.type) === "link_open" && ((_Na = tokens[i2 + 5]) == null ? void 0 : _Na.type) === "link_close" && ((_Oa = tokens[i2 + 6]) == null ? void 0 : _Oa.type) === "text" && tokens[i2 + 6].content === ")" && ((_Pa = tokens[i2 + 7]) == null ? void 0 : _Pa.type) === "strong_close") {
      const startMatch = (curToken.content || "").match(/^(\*+)\[(.*)$/);
      if (startMatch) {
        const finalLabel = (startMatch[2] || "") + startMatch[1];
        let href = ((_Sa = (_Ra = (_Qa = tokens[i2 + 3]) == null ? void 0 : _Qa.attrs) == null ? void 0 : _Ra.find((a2) => a2[0] === "href")) == null ? void 0 : _Sa[1]) || "";
        if (!href && ((_Ta = tokens[i2 + 4]) == null ? void 0 : _Ta.type) === "text") href = tokens[i2 + 4].content || "";
        const out = [];
        pushEmOpen(out, 2);
...
function fixStrongTokens(tokens) {
  ...
  for (let i$1 = 0; i$1 < tokens.length; i$1++) {
    ...
    if (type === "strong_open") {
      ...
      let j2 = i$1 - 1;
      while (j2 >= 0 && tokens[j2].type === "text" && tokens[j2].content === "") j2--;
      const preToken = tokens[j2];
      let k2 = i$1 + 1;
      while (k2 < tokens.length && tokens[k2].type === "text" && tokens[k2].content === "") k2++;
      const postToken = tokens[k2];
  ...
  for (let i2 = 0; i2 < tokens.length; i2++) {
    ...
    if ((t0 == null ? void 0 : t0.type) === "strong_open" && (t1 == null ? void 0 : t1.type) === "text" && (t2 == null ? void 0 : t2.type) === "strong_close" && (t3 == null ? void 0 : t3.type) === "strong_open" && (t4 == null ? void 0 : t4.type) === "math_inline" && (t5 == null ? void 0 : t5.type) === "strong_close") {
      ...
      if (close) {
        ...
        out.push(t1);
        out.push(t4);
        for (let j2 = i2 + 6; j2 < close.index; j2++) out.push(tokens[j2]);
...
function applyFixTableTokens(md) {
  md.core.ruler.after("block", "fix_table_tokens", (state) => {
    ...
    const s = state;
    try {
      const fixed = fixTableTokens(s.tokens ?? [], !!((_a2 = s.env) == null ? void 0 : _a2.__markstreamFinal), s.src ?? "");
...
var ESCAPED_TEX_BRACE_COMMANDS = TEX_BRACE_COMMANDS.map((c2) => c2.replace(/[.*+?^${}()|[\\]"\]/g, "\\$&")).join("|");
...
var MARKSTREAM_MATH_PLUGIN_APPLIED = "__markstreamMathPluginApplied";
var TOLERANT_BOUNDARY_SCAN_MAX_LINES = 80;
var TOLERANT_BOUNDARY_SCAN_MAX_CHARS = 2e4;
var TOLERANT_BOUNDARY_SCAN_TAIL_CHARS = TOLERANT_BOUNDARY_SCAN_MAX_CHARS + 4096;
...
var ESCAPED_KATEX_COMMANDS = KATEX_COMMANDS.slice().sort((a2, b2) => b2.length - a2.length).map((c2) => c2.replace(/[.*+?^${}()|[\\]\\\]/g, "\\$&")).join("|");
var CONTROL_CHARS_CLASS = "[	\r\b\f\v]";
var ESCAPED_MKATWX_COMMANDS = new RegExp(`([^\\\\])(${ANY_COMMANDS.map((c2) => c2).join("|")})+`, "g");
...
var FACTORIAL_PRECEDING_RE = /[\p{L}\p{M}\p{N}\p{Pe}\p{Pf}'′″‴|‖]/u;
...
function getMathRegex(commands) {
  ...
  const arr = [...commands];
  arr.sort((a2, b2) => b2.length - a2.length);
  const key = arr.join("");
  const cached = MATH_RE_CACHE.get(key);
  if (cached) return cached;
  const commandPattern = `(?:${arr.map((c2) => c2.replace(/[.*+?^${}()|[\\]\\"\]/g, "\\$&")).join("|")})`;
...
function getBraceCmdRegex(useDefault, commands) {
  ...
  if (!useDefault) arr.sort((a2, b2) => b2.length - a2.length);
  const key = useDefault ? "__default__" : arr.join("");
  const cached = BRACE_CMD_RE_CACHE.get(key);
  if (cached) return cached;
  const braceEscaped = useDefault ? [ESCAPED_TEX_BRACE_COMMANDS, ESCAPED_KATEX_COMMANDS].filter(Boolean).join("|") : [arr.map((c2) => c2.replace(/[.*+?^${}()|[\\]\\\]/g, "\\$&")).join("|"), ESCAPED_TEX_BRACE_COMMANDS].filter(Boolean).join("|");
...
function countUnescapedStrong(s) {
  const re = /(^|[^\\])(__|\*\*)/g;
  let c2 = 0;
  while (re.exec(s) !== null) c2++;
  return c2;
}
function escapeStandaloneExclamation(value) {
  return value.replace(/(^|[^\\])!+/gu, (match2, prefix) => {
    if (prefix && FACTORIAL_PRECEDING_RE.test(prefix)) return match2;
    const marks = prefix ? match2.slice(prefix.length) : match2;
...
function findLastUnescapedStrongMarker(s) {
  ...
  let m2;
  let last = null;
  while ((m2 = re.exec(s)) !== null) last = {
    marker: m2[2],
    index: m2.index + (((_a2 = m2[1]) == null ? void 0 : _a2.length) ?? 0)
  };
  return last;
}
function normalizeStandaloneBackslashT(s, opts) {
  ...
  let out = s.replace(re, (m2, control, cmd, offset, str) => {
    ...
    if (cmd && commands.includes(cmd)) {
      ...
      if (prev === "\\" || prev && /\w/.test(prev)) return m2;
      return `\\${cmd}`;
    }
    return m2;
  });
...
function buildCodeSpanRanges(src) {
  ...
  while (i2 < src.length) {
    ...
    while (j2 < src.length) {
      ...
      let runLen = 1;
      while (j2 + runLen < src.length && src[j2 + runLen] === "`") runLen++;
      if (runLen === openLen) {
        closeStart = j2;
        break;
      }
...
function buildImageRanges(src, allowIncomplete = false) {
  ...
  while (i2 < src.length - 1) {
    if (src[i2] === "!" && src[i2 + 1] === "[") {
      ...
      let j2 = i2 + 2;
      let labelDepth = 1;
      while (j2 < src.length && labelDepth > 0) {
        if (src[j2] === "\\" && j2 + 1 < src.length) {
          j2 += 2;
          continue;
        }
        if (src[j2] === "[") labelDepth++;
        else if (src[j2] === "]") labelDepth--;
        j2++;
      }
      if (labelDepth === 0 && j2 < src.length && src[j2] === "(") {
        let k2 = j2 + 1;
        let depth = 1;
        while (k2 < src.length && depth > 0) {
          if (src[k2] === "\\" && k2 + 1 < src.length) {
            k2 += 2;
            continue;
          }
          if (src[k2] === "(") depth++;
          else if (src[k2] === ")") depth--;
          k2++;
        }
        if (depth === 0) {
          ranges.push([start, k2]);
          i2 = k2;
          continue;
        }
        if (allowIncomplete) {
...
function applyMath(md, mathOpts) {
  ...
  const mathInline = (state, silent) => {
    ...
    const allowLoading = !((_a2 = s == null ? void 0 : s.env) == null ? void 0 : _a2.__markstreamFinal);
  ...
  const mathBlock = (state, startLine, endLine, silent) => {
    ...
    for (const [open, close] of delimiters) if (lineText.startsWith(open)) if (open.includes("[")) {
      ...
      if (mathOpts == null ? void 0 : mathOpts.strictDelimiters) {
        if (lineText.replace("\\", "") === "[") {
...
var HTML_LINK_OPEN_RE = /^<a[>\s]/i;
var HTML_LINK_CLOSE_RE = /^<\/a\s*>/i;
function inlineTokenMayNeedLinkify(token, linkify$2) {
  var _a2, _b;
  if ((token == null ? void 0 : token.type) !== "inline") return false;
  const children = token.children;
  if (!Array.isArray(children) || children.length === 0) return linkify$2.pretest(String(token.content ?? ""));
  let htmlLinkLevel = 0;
  for (let index = children.length - 1; index >= 0; index--) {
    const currentToken = children[index];
    if ((currentToken == null ? void 0 : currentToken.type) === "link_close") {
      index--;
      while (index >= 0 && ((_a2 = children[index]) == null ? void 0 : _a2.level) !== currentToken.level && ((_b = children[index]) == null ? void 0 : _b.type) !== "link_open") index--;
      continue;
    }
    if ((currentToken == null ? void 0 : currentToken.type) === "html_inline") {
    ...
    if (htmlLinkLevel > 0) continue;
    if ((currentToken == null ? void 0 : currentToken.type) === "text" && linkify$2.pretest(String(currentToken.content ?? ""))) return true;
  }
  return false;
}
function applyLinkifyCandidateFilter(md) {
  var _a2, _b, _c;
  const ruler = (_a2 = md.core) == null ? void 0 : _a2.ruler;
  const original = (_c = (_b = ruler.getNamedRules) == null ? void 0 : _b.call(ruler).find((rule) => rule.name === "linkify")) == null ? void 0 : _c.fn;
  if (typeof original !== "function") return;
  ruler.at("linkify", (state) => {
    var _a3, _b2;
    if (!((_b2 = (_a3 = state.md) == null ? void 0 : _a3.options) == null ? void 0 : _b2.linkify)) return;
...
function applyInlineUrlValidation(md) {
  ...
  const rules = (_a2 = ruler.getNamedRules) == null ? void 0 : _a2.call(ruler);
  const originalLink = (_b = rules == null ? void 0 : rules.find((rule) => rule.name === "link")) == null ? void 0 : _b.fn;
  const originalImage = (_c = rules == null ? void 0 : rules.find((rule) => rule.name === "image")) == null ? void 0 : _c.fn;
  ...
  ruler.at("link", (...args) => {
    ...
    const validateLink$1 = (activeMd == null ? void 0 : activeMd.validateLink) === imageValidateLink ? (_a3 = activeMd.options) == null ? void 0 : _a3.validateLink : activeMd == null ? void 0 : activeMd.validateLink;
...
function factory(opts = {}) {
  ...
  if (!hasCustomValidateLink) md.set({ validateLink: (url) => !isUnsafeHtmlUrl(url, {
    tagName: "a",
    attrName: "href"
  }) });
...
function splitUnifiedDiff(content, closed) {
  ...
  const processLine = (rawLine) => {
    const line = rawLine;
    if (DIFF_HEADER_PREFIXES.some((p2) => line.startsWith(p2))) return;
    if (line.startsWith("-")) {
...
function getCustomHtmlSourceMeta(token) {
  ...
  const raw = meta == null ? void 0 : meta.markstreamCustomHtmlRaw;
  const inner = meta == null ? void 0 : meta.markstreamCustomHtmlInner;
...
function parseHtmlInlineCodeToken(token, tokens, i2, parseInlineTokens$1, raw, pPreToken, options) {
  ...
  if (customTagSet == null ? void 0 : customTagSet.has(tag)) {
    ...
    return [{
      ...
      raw: (sourceMeta == null ? void 0 : sourceMeta.raw) ?? content,
...
function parseLinkToken(tokens, startIndex, options) {
  ...
  if (((_a2 = tokens[i2]) == null ? void 0 : _a2.type) === "link_close") loading = false;
...
var CJK_OPENING_PUNCTUATION_RE = /^[《「『【〔〖〘〚〈（［｛“‘﹁﹃﹙﹛﹝]$/u;
var CJK_CLOSING_PUNCTUATION_RE = /^[》」』】〕〗〙〛〉）］｝”’﹂﹄﹚﹜﹞]$/u;
var AUTOLINK_PROTOCOL_RE = /^(?:https?:\/\/|mailto:|ftp:\/\/)/i;
var AUTOLINK_GENERIC_RE = /:\/\//;
var INLINE_TEXT_MARKER_BACKSLASH = 1;
var INLINE_TEXT_MARKER_ASTERISK = 2;
var INLINE_TEXT_MARKER_UNDERSCORE = 4;
var INLINE_TEXT_MARKER_TILDE = 8;
var INLINE_TEXT_MARKER_BACKTICK = 16;
var INLINE_TEXT_MARKER_OPEN_BRACKET = 32;
var INLINE_TEXT_MARKER_CLOSE_BRACKET = 64;
var INLINE_TEXT_MARKER_BANG = 128;
var INLINE_TEXT_MARKER_DOLLAR = 256;
var INLINE_TEXT_MARKER_PIPE = 512;
var INLINE_TEXT_MARKER_OPEN_PAREN = 1024;
var INLINE_CANDIDATE_MARKERS = 1982;
...
function isCjkOpeningPunctuation(ch, previous) {
  return !!ch && !!previous && new RegExp("^\\p{Script=Han}$", "u").test(previous) && CJK_OPENING_PUNCTUATION_RE.test(ch);
}
...
function parseInlineTokens(tokens, raw, pPreToken, options) {
  ...
  const linkifyDemotionContext = {
    filename: (inheritedContext == null ? void 0 : inheritedContext.filename) || inferredContext.filename,
    explicitFilename: (inheritedContext == null ? void 0 : inheritedContext.explicitFilename) || inferredContext.explicitFilename,
    marketTicker: (inheritedContext == null ? void 0 : inheritedContext.marketTicker) || inferredContext.marketTicker
  };
  ...
  function handleEmphasisAndStrikethrough(content, token) {
    ...
    markerCandidates.sort((a2, b2) => {
      if (a2.index !== b2.index) return a2.index - b2.index;
      if (a2.type === b2.type) return 0;
      if (a2.type === "strong") return -1;
      if (b2.type === "strong") return 1;
      return 0;
    });
    ...
    if (nextMarker.type === "strong") {
      ...
      if (raw && openIdx === 0) {
        ...
        if (rawHasEscapedAsteriskAtStart) {
          let escapedCount = 0;
          let j2 = 0;
          while (j2 < raw.length && escapedCount < asteriskCount) if (raw[j2] === "\\" && j2 + 1 < raw.length && raw[j2 + 1] === "*") {
            escapedCount += 1;
            j2 += 2;
          } else if (raw[j2] === "*") break;
          else j2++;
          if (escapedCount >= 2) {
  ...
  function handleInlineCodeContent(content, _token) {
    ...
    const findFirstUnescapedBacktick = (src) => {
      for (let idx = 0; idx < src.length; idx++) {
        if (src[idx] !== "`") continue;
        let slashCount = 0;
        for (let j2 = idx - 1; j2 >= 0 && src[j2] === "\\"; j2--) slashCount++;
    ...
    let runLen = 1;
    for (let k2 = codeStart + 1; k2 < content.length && content[k2] === "`"; k2++) runLen++;
    ...
    if (codeEnd === -1) {
      ...
      let merged = content;
      for (let j2 = i2 + 1; j2 < tokens.length; j2++) merged += String((tokens[j2].content ?? "") + (tokens[j2].markup ?? ""));
      i2 = tokens.length - 1;
  ...
  function tryReparseCollapsedInlineText(rawContent) {
    ...
    const children = (((_a2 = reparsed.find((token) => (token == null ? void 0 : token.type) === "inline")) == null ? void 0 : _a2.children) ?? []).filter((child) => !((child == null ? void 0 : child.type) === "text" && String(child.content ?? "") === ""));
  ...
  function pushInlineTextContent(content, token) {
    ...
    if (parsed.length === 1 && ((_a2 = parsed[0]) == null ? void 0 : _a2.type) === "text") {
  ...
  function isMarkdownLinkBeforeLinkifiedUrl(content) {
    ...
    return ((_a2 = tokens[i2 + 1]) == null ? void 0 : _a2.type) === "link_open" && ((_b = tokens[i2 + 1]) == null ? void 0 : _b.markup) === "linkify" && ((_c = tokens[i2 + 2]) == null ? void 0 : _c.type) === "text" && ((_d = tokens[i2 + 3]) == null ? void 0 : _d.type) === "link_close" && ((_e = tokens[i2 + 4]) == null ? void 0 : _e.type) === "text" && String(((_f = tokens[i2 + 4]) == null ? void 0 : _f.content) ?? "").startsWith(")");
  }
  ...
  function handleToken(token) {
    ...
    switch (token.type) {
      ...
      default: {
        ...
        else if (recoverMarkdownImageFromLoadingImageTail(token)) i2++;
  ...
  function commitTextNode(content, token, preToken, nextToken, markerFlags = getInlineTextMarkerFlags(content)) {
    ...
    const maybeMath = (preToken == null ? void 0 : preToken.tag) === "br" && ((_a2 = tokens[i2 - 2]) == null ? void 0 : _a2.content) === "[";
    if (!nextToken) textNode.content = stripTrailingMidStateMarker(textNode.content, token, markerFlags);
  ...
  function handleTextToken(token) {
    ...
    if (!nextToken && (markerFlags & INLINE_TEXT_MARKER_OPEN_PAREN) !== 0 && /[^\]]\s*\(\s*$/.test(content)) content = content.replace(/\(\s*$/, "");
    if (!content) {
      i2++;
      return;
    }
    ...
    if (!((markerFlags & INLINE_CANDIDATE_MARKERS) !== 0)) {
      commitTextNode(content, token, tokens[i2 - 1], nextToken, markerFlags);
      i2++;
      return;
    }
    if ((markerFlags & INLINE_TEXT_MARKER_OPEN_BRACKET) !== 0 && handleCheckboxLike(content)) return;
    const preToken = tokens[i2 - 1];
    if ((markerFlags & INLINE_TEXT_MARKER_OPEN_BRACKET) !== 0 && content === "[" && !((_b = nextToken == null ? void 0 : nextToken.markup) == null ? void 0 : _b.includes("*")) && !hasEscapedMarkup(token, "\\[") || (markerFlags & INLINE_TEXT_MARKER_CLOSE_BRACKET) !== 0 && content === "]" && !((_c = preToken == null ? void 0 : preToken.markup) == null ? void 0 : _c.includes("*")) && !hasEscapedMarkup(token, "\\]")) {
      i2++;
      return;
    }
    ...
    if ((markerFlags & INLINE_TEXT_MARKER_OPEN_BRACKET) !== 0 && (((_d = tokens[i2 + 1]) == null ? void 0 : _d.type) !== "link_open" || isMarkdownLinkBeforeLinkifiedUrl(content)) && handleInlineLinkContent(content, token)) return;
  ...
  function handleLinkOpen(token) {
    ...
    if (token.markup === "linkify" && !isDecodedFromRawPunycode(linkText, node.href, raw) && shouldDemoteFilenameLikeLinkify(linkText, internalOptions == null ? void 0 : internalOptions.__linkifyDemotionContext)) {
      pushText(linkText, linkText);
      return;
    }
    const hasSingleTextChild = node.children.length === 1 && ((_a2 = node.children[0]) == null ? void 0 : _a2.type) === "text";
  ...
  function recoverMarkdownImageFromLoadingImageTailLink(link$1, nextIndex) {
    const previous = result[result.length - 1];
    if ((previous == null ? void 0 : previous.type) !== "image" || previous.src || !previous.loading || !String(previous.raw ?? "").endsWith("](")) return false;
    const nextToken = tokens[nextIndex];
    const nextContent = String((nextToken == null ? void 0 : nextToken.content) ?? "");
    if ((nextToken == null ? void 0 : nextToken.type) !== "text" || !nextContent.startsWith(")")) return false;
    result.pop();
    currentTextNode = null;
  ...
  function recoverMarkdownImageFromTrailingBang(token) {
    ...
    const linkToken = token;
    const alt = String(linkToken.text ?? ((_a2 = linkToken.children) == null ? void 0 : _a2.map((child) => String((child == null ? void 0 : child.content) ?? (child == null ? void 0 : child.raw) ?? "")).join("")) ?? "");
    const href = String(linkToken.href ?? "");
  ...
  function recoverOuterImageLinkMidStateFromText(content) {
    ...
    const previousLink = (previous == null ? void 0 : previous.type) === "link" && Array.isArray(previousWithChildren.children) && previousWithChildren.children.length === 1 && ((_a2 = previousWithChildren.children[0]) == null ? void 0 : _a2.type) === "image" ? result.pop() : null;
  ...
  function shouldTreatLinkOpenAsTextInEscapedOuterImageTail() {
    ...
    return ((_a2 = tokens[i2 - 2]) == null ? void 0 : _a2.type) === "image" && ((_b = tokens[i2 - 1]) == null ? void 0 : _b.type) === "text" && String(tokens[i2 - 1].content ?? "") === "](" && (outerOpenToken == null ? void 0 : outerOpenToken.type) === "text" && String(outerOpenToken.content ?? "").endsWith("[") && hasEscapedMarkup(outerOpenToken, "\\[");
  }
        ...
        if ((last == null ? void 0 : last.type) === "text") {
          ...
          if (lastContent.startsWith(")")) {
            ...
            if (trailingAfterClose) {
              ...
              ensureWorkingTokens()[i2 + 4] = trailingToken;
            } else index++;
  ...
  function handleCheckboxLike(content) {
    ...
    const w2 = content.slice(1).match(/[^\s\]]/);
    if (w2 === null) {
      i2++;
      return true;
    }
    if (w2 && /x/i.test(w2[0])) {
      const checked = w2[0] === "x" || w2[0] === "X";
      pushParsed({
        type: "checkbox_input",
        checked,
        raw: checked ? "[x]" : "[ ]"
...
function mapSourceLineRange(startLine, endLine, options) {
  const mapper = options == null ? void 0 : options.__sourceLineMapper;
  if (!mapper) return {
    startLine,
    endLine
  };
...
function lineAtOffset(source, offset) {
  ...
  let line = 0;
  for (let i2 = 0; i2 < target; i2++) if (source[i2] === "\n") line++;
  return line;
}
...
function readSourceMap(token, options) {
  const map$1 = token == null ? void 0 : token.map;
...
function applyNodeSourceMap(node, token, options) {
  if (!(options == null ? void 0 : options.includeSourceMap)) return node;
...
function parseList(tokens, index, options) {
  ...
  let j2 = index + 1;
  while (j2 < tokens.length && tokens[j2].type !== "bullet_list_close" && tokens[j2].type !== "ordered_list_close") if (tokens[j2].type === "list_item_open") {
    const itemChildren = [];
    let k2 = j2 + 1;
    while (k2 < tokens.length && tokens[k2].type !== "list_item_close") if (tokens[k2].type === "paragraph_open") {
      const originalContentToken = tokens[k2 + 1];
      ...
      if (options == null ? void 0 : options.includeSourceMap) applyNodeSourceMap(paragraphNode, tokens[k2], options);
      itemChildren.push(paragraphNode);
      linkifyContext.remember(paragraphRaw);
      k2 += 3;
    } else if (tokens[k2].type === "blockquote_open") {
      const [blockquoteNode, newIndex] = parseBlockquote(tokens, k2, linkifyContext.options());
      itemChildren.push(blockquoteNode);
      linkifyContext.remember(blockquoteNode.raw);
      k2 = newIndex;
    } else if (tokens[k2].type === "bullet_list_open" || tokens[k2].type === "ordered_list_open") {
      const [nestedListNode, newIndex] = parseList(tokens, k2, linkifyContext.options());
      itemChildren.push(nestedListNode);
      linkifyContext.remember(nestedListNode.raw);
      k2 = newIndex;
    } else {
      const handled = parseCommonBlockToken(tokens, k2, linkifyContext.options(), containerTokenHandlers);
      if (handled) {
    ...
    if (options == null ? void 0 : options.includeSourceMap) applyNodeSourceMap(listItemNode, tokens[j2], options);
  ...
  if (options == null ? void 0 : options.includeSourceMap) applyNodeSourceMap(listNode, token, options);
  return [listNode, j2 + 1];
}
function parseAdmonition(tokens, index, match2, options) {
  ...
  const linkifyContext = createLinkifyDemotionContextTracker(options, true);
  let j2 = index + 1;
  while (j2 < tokens.length && tokens[j2].type !== "container_close") if (tokens[j2].type === "paragraph_open") {
    const contentToken = tokens[j2 + 1];
    if (contentToken) {
      const paragraphNode = {
        type: "paragraph",
      ...
      if (options == null ? void 0 : options.includeSourceMap) applyNodeSourceMap(paragraphNode, tokens[j2], options);
      admonitionChildren.push(paragraphNode);
      linkifyContext.remember(paragraphNode.raw);
    }
    j2 += 3;
  } else if (tokens[j2].type === "bullet_list_open" || tokens[j2].type === "ordered_list_open") {
    const [listNode, newIndex] = parseList(tokens, j2, linkifyContext.options());
    if (options == null ? void 0 : options.includeSourceMap) applyNodeSourceMap(listNode, tokens[j2], options);
    admonitionChildren.push(listNode);
    linkifyContext.remember(listNode.raw);
    j2 = newIndex;
  } else if (tokens[j2].type === "blockquote_open") {
    const [blockquoteNode, newIndex] = parseBlockquote(tokens, j2, linkifyContext.options());
    if (options == null ? void 0 : options.includeSourceMap) applyNodeSourceMap(blockquoteNode, tokens[j2], options);
    admonitionChildren.push(blockquoteNode);
    linkifyContext.remember(blockquoteNode.raw);
    j2 = newIndex;
  } else {
    const handled = parseBasicBlockToken(tokens, j2, linkifyContext.options());
    if (handled) {
...
function parseContainer(tokens, index, options) {
  ...
  while (j2 < tokens.length && tokens[j2].type !== "container_close" && !closeType.test(tokens[j2].type)) if (tokens[j2].type === "paragraph_open") {
    const contentToken = tokens[j2 + 1];
    if (contentToken) {
      const childrenArr = contentToken.children || [];
      let i2 = -1;
      for (let k2 = childrenArr.length - 1; k2 >= 0; k2--) {
        const t2 = childrenArr[k2];
      ...
      const paragraphNode = {
        type: "paragraph",
        children: parseInlineTokens((i2 !== -1 ? childrenArr.slice(0, i2) : childrenArr) || [], void 0, void 0, linkifyContext.options()),
...
function parseBlockquote(tokens, index, options) {
  ...
  const linkifyContext = createLinkifyDemotionContextTracker(options, true);
  let j2 = index + 1;
  while (j2 < tokens.length && tokens[j2].type !== "blockquote_close") {
    const token = tokens[j2];
    switch (token.type) {
      case "paragraph_open": {
        const contentToken = tokens[j2 + 1];
        const paragraphNode = {
        ...
        if (options == null ? void 0 : options.includeSourceMap) applyNodeSourceMap(paragraphNode, token, options);
      ...
      case "blockquote_open": {
        const [nestedBlockquote, newIndex] = parseBlockquote(tokens, j2, linkifyContext.options());
      ...
      default: {
        const handled = parseCommonBlockToken(tokens, j2, linkifyContext.options(), containerTokenHandlers);
        if (handled) {
  ...
  if (options == null ? void 0 : options.includeSourceMap) applyNodeSourceMap(blockquoteNode, tokens[index], options);
  return [blockquoteNode, j2 + 1];
}
function parseCodeBlock(token) {
  ...
  if (match2 == null ? void 0 : match2[1]) code$1 = contentStr.replace(/<antArtifact[^>]*>/g, "").replace(/<\/antArtifact>/g, "");
  const hasMap = Array.isArray(token.map) && token.map.length === 2;
  return {
    type: "code_block",
...
function parseDefinitionList(tokens, index, options) {
  ...
  let definitionNodes = [];
  const linkifyContext = createLinkifyDemotionContextTracker(options, true);
  while (j2 < tokens.length && tokens[j2].type !== "dl_close") if (tokens[j2].type === "dt_open") {
    const termToken = tokens[j2 + 1];
    ...
    j2 += 3;
  } else if (tokens[j2].type === "dd_open") {
    let k2 = j2 + 1;
    definitionNodes = [];
    while (k2 < tokens.length && tokens[k2].type !== "dd_close") if (tokens[k2].type === "paragraph_open") {
      const contentToken = tokens[k2 + 1];
...
function parseFootnote(tokens, index, options) {
  ...
  const linkifyContext = createLinkifyDemotionContextTracker(options, true);
  let j2 = index + 1;
  while (j2 < tokens.length && tokens[j2].type !== "footnote_close") if (tokens[j2].type === "paragraph_open") {
    const contentToken = tokens[j2 + 1];
    const children = contentToken.children ? [...contentToken.children] : [];
    if (tokens[j2 + 2].type === "footnote_anchor") children.push(tokens[j2 + 2]);
    const paragraphNode = {
      type: "paragraph",
...
function extractAlign(attrs) {
  ...
  for (const a2 of attrs) {
    ...
    if (key === "style") {
      const m2 = /text-align\s*:\s*(left|right|center)/i.exec(value);
...
function parseOptionsForTableCell(options, headerRaw, rowContext) {
  ...
  return {
    ...options,
    __linkifyDemotionContext: {
      filename: (inheritedContext == null ? void 0 : inheritedContext.filename) || (cellContext == null ? void 0 : cellContext.filename),
      explicitFilename: (inheritedContext == null ? void 0 : inheritedContext.explicitFilename) || (cellContext == null ? void 0 : cellContext.explicitFilename),
      marketTicker: (inheritedContext == null ? void 0 : inheritedContext.marketTicker) || (cellContext == null ? void 0 : cellContext.marketTicker)
    }
  };
}
function parseTable(tokens, index, options) {
  ...
  const rows = [];
  let isHeader = false;
  while (j2 < tokens.length && tokens[j2].type !== "table_close") if (tokens[j2].type === "thead_open") {
    isHeader = true;
    j2++;
  } else if (tokens[j2].type === "thead_close") {
    isHeader = false;
    j2++;
  } else if (tokens[j2].type === "tbody_open" || tokens[j2].type === "tbody_close") j2++;
  else if (tokens[j2].type === "tr_open") {
    const cells = [];
    let k2 = j2 + 1;
    let rowContext;
    while (k2 < tokens.length && tokens[k2].type !== "tr_close") if (tokens[k2].type === "th_open" || tokens[k2].type === "td_open") {
      const isHeaderCell = tokens[k2].type === "th_open";
      const contentToken = tokens[k2 + 1];
      const content = String(contentToken.content ?? "");
      const align = extractAlign(tokens[k2].attrs);
      const cellIndex = cells.length;
      const isBodyCell = !isHeaderCell && !isHeader;
      const headerRaw = isBodyCell ? (_a2 = headerRow == null ? void 0 : headerRow.cells[cellIndex]) == null ? void 0 : _a2.raw : void 0;
      cells.push({
        type: "table_cell",
  ...
  return [{
    ...
    header: headerRow,
    rows,
    loading: tokenLoading && !(options == null ? void 0 : options.final) && rows.length === 0,
...
function parseVmrContainer(tokens, index, options) {
  ...
  const linkifyContext = createLinkifyDemotionContextTracker(options, true);
  let j2 = index + 1;
  while (j2 < tokens.length && tokens[j2].type !== "vmr_container_close") if (tokens[j2].type === "paragraph_open") {
    const contentToken = tokens[j2 + 1];
    if (contentToken) {
      const paragraphNode = {
        type: "paragraph",
  ...
  const hasCloseToken = j2 < tokens.length && tokens[j2].type === "vmr_container_close";
  const closed = hasCloseToken && ((_a2 = openToken.meta) == null ? void 0 : _a2.unclosed) !== true || !!(options == null ? void 0 : options.final);
  let raw = `::: ${name}`;
  if (Object.keys(containerAttrs).length > 0) raw += ` ${JSON.stringify(containerAttrs)}`;
  raw += "\n";
  if (children.length > 0) {
    raw += openToken.raw ?? children.map((c2) => c2.raw).join("\n");
    raw += "\n";
  }
  raw += ":::";
  return [{
    ...
    children,
    raw
  }, hasCloseToken ? j2 + 1 : j2];
}
function applyPairedBlockSourceMap(node, openToken, closeToken, options) {
  if (closeToken == null ? void 0 : closeToken.type.endsWith("_close")) {
...
function parseBasicBlockToken(tokens, index, options) {
  var _a2, _b;
  const token = tokens[index];
  const includeSourceMap = (options == null ? void 0 : options.includeSourceMap) === true;
  switch (token.type) {
    case "heading_open": {
    ...
    case "html_block": {
      ...
      if (htmlBlockNode.tag && ((_a2 = tagSets == null ? void 0 : tagSets.customTagSet) == null ? void 0 : _a2.has(htmlBlockNode.tag))) {
        ...
        const attrRegex = /\s([\w:-]+)(?:\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s"'>]+)))?/g;
        let m2;
        while ((m2 = attrRegex.exec(openTag)) !== null) {
          const name = m2[1];
          if (!name || name.toLowerCase() === tag) continue;
          const value = m2[2] || m2[3] || m2[4] || "";
...
function parseCommonBlockToken(tokens, index, options, handlers) {
  ...
  switch (token.type) {
    ...
    case "container_error_open":
      if (handlers == null ? void 0 : handlers.parseContainer) {
...
var streamParseEnvCache = /* @__PURE__ */ new WeakMap();
var internalNodeSourceRanges = /* @__PURE__ */ new WeakMap();
var sourceLineOffsetsCache = /* @__PURE__ */ new WeakMap();
var siblingHtmlChildrenCache = /* @__PURE__ */ new WeakMap();
function recordInternalNodeSourceRange(node, token, options) {
  const map$1 = token == null ? void 0 : token.map;
  const source = options == null ? void 0 : options.__sourceMarkdown;
...
var tolerantMathBoundaryStreamCache = /* @__PURE__ */ new WeakMap();
var pendingExplicitMathTailCache = /* @__PURE__ */ new WeakMap();
var TOLERANT_BOUNDARY_SPLIT_OPENERS = ["$$", "\\["];
var STREAMING_ADMONITION_OPEN_RE = /(^|\r?\n)[\t ]*:::[\t ]*(?:warning|info|note|tip|danger|caution|error)(?=[\t ]|\r?\n|$)[^\r\n]*(?:\r?\n[\t ]*)*$/;
var structuredStreamCache = /* @__PURE__ */ new WeakMap();
var topLevelStreamParseMode = /* @__PURE__ */ new WeakMap();
var REUSABLE_INLINE_TOKEN_TYPES = /* @__PURE__ */ new Set([
  "code_inline",
  "em_close",
  "em_open",
  "emoji",
...
var REUSABLE_TOP_LEVEL_PAIRED_TOKEN_TYPES = /* @__PURE__ */ new Map([
...
var REUSABLE_TOP_LEVEL_SINGLE_TOKEN_TYPES = /* @__PURE__ */ new Set([
  "code_block",
  "fence",
  "hr",
  "math_block"
...
function hasOnlyReusableInlineTokens(tokens) {
...
function getReusableTopLevelTokenGroups(tokens) {
  var _a2, _b;
  const groupStarts = [];
  let mixed = false;
  let index = 0;
  while (index < tokens.length) {
    ...
    if (closeType) {
      ...
      if (((_a2 = tokens[groupEnd - 1]) == null ? void 0 : _a2.type) !== closeType) return null;
      if (token.type === "paragraph_open" || token.type === "heading_open") {
        if (groupEnd !== index + 3 || ((_b = tokens[index + 1]) == null ? void 0 : _b.type) !== "inline") return null;
      } else mixed = true;
    ...
    for (let tokenIndex = index; tokenIndex < groupEnd; tokenIndex++) {
      ...
      if (!Array.isArray(children) || !hasOnlyReusableInlineTokens(children)) return null;
    }
...
function canReuseStructuredStreamNodes(options) {
  var _a2;
  return options.__reuseStableTopLevelNodes === true && options.final !== true && !options.preTransformTokens && !options.postTransformTokens && !options.postTransformNodes && !((_a2 = options.customHtmlTags) == null ? void 0 : _a2.length) && options.includeSourceMap !== true;
}
...
function hasStableStructuredStreamGroupBoundaries(previous, tokens, groupStarts, stableGroupCount) {
  for (let index = 0; index < stableGroupCount; index++) {
    ...
    if (!boundary || boundary.firstToken !== tokens[start] || boundary.lastToken !== tokens[end - 1] || boundary.tokenCount !== end - start) return false;
  }
  return true;
}
function processTopLevelTokensWithReuse(md, source, tokens, options, timing) {
  ...
  if (!(shouldUseTopLevelStreamParse(md, options) && canReuseStructuredStreamNodes(options) && groups !== null)) {
  ...
  const stableGroupCount = previous && groups.mixed ? Math.min(previous.stableGroupCount, Math.max(0, previous.groupBoundaries.length - 1)) : (previous == null ? void 0 : previous.stableGroupCount) ?? 0;
  if (previous && stableGroupCount > 0 && previous.requireClosingStrong === options.requireClosingStrong && source.startsWith(previous.source) && groupStarts.length >= stableGroupCount && (mode === "append" || mode === "tail") && hasStableStructuredStreamGroupBoundaries(previous, tokens, groupStarts, stableGroupCount)) {
...
function shouldUseTopLevelStreamParse(md, options) {
  ...
  return internalOptions.__disableStreamParse !== true && md.__markstreamHasCustomParserExtensions !== true && (streamParse === true || streamParse === "auto" && options.final !== true) && (stream == null ? void 0 : stream.enabled) === true && typeof stream.parse === "function";
}
function shouldResetTopLevelStreamCacheForFinalAutoParse(md, options) {
  const internalOptions = options;
  const streamParse = options.streamParse ?? "auto";
  const stream = md.stream;
  return options.final === true && streamParse === "auto" && internalOptions.__disableStreamParse !== true && md.__markstreamHasCustomParserExtensions !== true && (stream == null ? void 0 : stream.enabled) === true && typeof stream.reset === "function";
}
...
function stripMarkdownListPrefix(line) {
  ...
  const marker = (_a2 = /^(?:[-+*]|\d{1,9}[.)])(?=[\t ]|$)/.exec(rest)) == null ? void 0 : _a2[0];
...
function isInsideOpenStandardHtmlBlockBeforeOffset(markdown, offset) {
  ...
  const isNameChar = (ch) => {
    ...
    return c2 >= 65 && c2 <= 90 || c2 >= 97 && c2 <= 122 || c2 >= 48 && c2 <= 57 || ch === "_" || ch === "-" || ch === ":";
  };
  ...
  while (index < offset) {
    ...
    if (indent) {
      ...
      if (inComment) inComment = !rest.includes("-->");
        ...
        if (tagInfo) {
          if (tagInfo.closing) {
            for (let i2 = stack.length - 1; i2 >= 0; i2--) if (stack[i2] === tagInfo.tag) {
              stack.length = i2;
              break;
            }
...
function isInsideOpenCustomHtmlBlockBeforeOffset(markdown, offset, customHtmlTags) {
  if (!(customHtmlTags == null ? void 0 : customHtmlTags.length)) return false;
...
function getStreamingAdmonitionOpenTailReplacement(markdown, customHtmlTags) {
  const match2 = STREAMING_ADMONITION_OPEN_RE.exec(markdown);
  if (!match2) return null;
  const separator = match2[1] ?? "";
  const lineStart = match2.index + separator.length;
  ...
  return `${markdown.slice(0, match2.index)}${separator}`;
}
...
function stripPendingExplicitMathTail(markdown, md) {
  ...
  const state = (previous == null ? void 0 : previous.source) === markdown ? previous.state : previous && markdown.startsWith(previous.source) ? updateExplicitBracketMathStreamState(previous.state, markdown.slice(previous.source.length), previous.source.length - previous.state.lineBuffer.length).state : scanExplicitBracketMathStreamState(markdown).state;
...
function syncTolerantMathBoundaryStreamCache(md, source) {
  ...
  const stream = md.stream;
  if (typeof (stream == null ? void 0 : stream.reset) !== "function") return;
  const owner = md;
  const previous = tolerantMathBoundaryStreamCache.get(owner);
  if ((previous == null ? void 0 : previous.source) === source) return;
...
function sameTokenMap(left, right) {
  const leftMap = left == null ? void 0 : left.map;
  const rightMap = right == null ? void 0 : right.map;
...
function isParagraphTokenTriplet(tokens, index) {
  var _a2, _b, _c;
  return ((_a2 = tokens[index]) == null ? void 0 : _a2.type) === "paragraph_open" && ((_b = tokens[index + 1]) == null ? void 0 : _b.type) === "inline" && ((_c = tokens[index + 2]) == null ? void 0 : _c.type) === "paragraph_close";
}
...
function shouldUseSyncParseForPendingTolerantMathBoundary(md) {
  ...
  return typeof (cache == null ? void 0 : cache.key) === "string" && cache.key.startsWith("pending:");
}
function parseTopLevelTokens(md, source, env, options) {
  var _a2, _b, _c, _d, _e;
  const owner = md;
  if ((_a2 = options.customHtmlTags) == null ? void 0 : _a2.length) env.__markstreamCustomHtmlTags = options.customHtmlTags;
  ...
  if (shouldFallbackDuplicateTolerantMathStreamTokens(md, source, tokens)) {
    (_c = (_b = md.stream) == null ? void 0 : _b.reset) == null ? void 0 : _c.call(_b);
    topLevelStreamParseMode.set(owner, "sync");
    return md.parse(source, env);
  }
  const stats = (_e = (_d = md.stream) == null ? void 0 : _d.stats) == null ? void 0 : _e.call(_d);
  topLevelStreamParseMode.set(owner, (stats == null ? void 0 : stats.lastMode) ?? "stream");
...
var RAW_TEXT_HTML_TAGS = /* @__PURE__ */ new Set([
  "iframe",
  "script",
  "style",
  "textarea",
...
function findNextHtmlBlockFromSource(source, tag, startIndex) {
  ...
  const readMarkup = (start$1) => {
    ...
    if (!(tagMatch == null ? void 0 : tagMatch[1])) return {
      closing: false,
      end: start$1 + 1,
      selfClosing: false,
      tag: ""
  ...
  const findRawTextClose = (rawTextTag, from) => {
    ...
    const match2 = closeRe.exec(source);
    if (!match2 || match2.index == null) return null;
    const markup = readMarkup(match2.index);
    return markup ? {
      start: match2.index,
      end: markup.end
    } : null;
  ...
  while (searchIndex < source.length) {
    ...
    if (!markup.closing && RAW_TEXT_HTML_TAGS.has(markup.tag)) {
      searchIndex = ((_a2 = findRawTextClose(markup.tag, markup.end)) == null ? void 0 : _a2.end) ?? source.length;
      continue;
    }
  ...
  while (index < source.length) {
    ...
    if (!markup.closing && RAW_TEXT_HTML_TAGS.has(markup.tag)) {
      index = ((_b = findRawTextClose(markup.tag, markup.end)) == null ? void 0 : _b.end) ?? source.length;
      continue;
    }
    index = markup.end;
  }
...
function findApproximateConsumedPrefixEnd(exact, approximate) {
  ...
  let i2 = 0;
  let j2 = 0;
  while (i2 < exact.length && j2 < approximate.length) {
    if (exact[i2] === approximate[j2]) {
      i2++;
      j2++;
      continue;
    }
  ...
  return j2 === approximate.length ? i2 : -1;
}
...
function splitSiblingHtmlBlockFragments(fragment) {
  ...
  while (cursor < fragment.length) {
    ...
    if (!(tagMatch == null ? void 0 : tagMatch[1])) return null;
...
function parseSiblingHtmlBlockChildren(blocks, md, options, final) {
  var _a2;
  const customHtmlTags = ((_a2 = options.customHtmlTags) == null ? void 0 : _a2.join("\0")) ?? "";
  const cacheOwner = md;
...
function structureGenericHtmlBlockChildren(nodes, md, options, final) {
  return nodes.map((node) => {
    ...
    const closeStart = (exact == null ? void 0 : exact.closeStart) ?? -1;
    const hasClose = (exact == null ? void 0 : exact.closed) === true && closeStart >= openEnd + 1;
...
function hasTopLevelHtmlBlock(nodes) {
  for (const node of nodes) if ((node == null ? void 0 : node.type) === "html_block") return true;
  return false;
}
...
function mergeSplitTopLevelHtmlBlocks(nodes, final, source, options) {
  ...
  for (let i2 = 0; i2 < merged.length; i2++) {
    ...
    if (nodePos !== -1 && exact.end < currentRawEnd && source.slice(nodePos, currentRawEnd) === currentRaw) {
      ...
      if (options == null ? void 0 : options.includeSourceMap) node.sourceMap = createSourceMapFromOffsets(source, nodePos, currentRawEnd, options);
      continue;
    }
    ...
    if (options == null ? void 0 : options.includeSourceMap) node.sourceMap = createSourceMapFromOffsets(source, exact.start, exact.end, options);
    ...
    const j2 = i2 + 1;
    while (j2 < merged.length) {
      if (exact.closed && isCloseOnlyHtmlBlockForTag(merged[j2], tag)) {
        merged.splice(j2, 1);
        continue;
      }
      const nextRaw = getMergeableNodeRaw(merged[j2]);
      if (!nextRaw) break;
      const nextPos = exact.raw.indexOf(nextRaw, tailCursor);
      if (nextPos === -1) {
        if (canFindNodeRawAfterSourceIndex(source, exact.end, nextRaw)) break;
        const range = internalNodeSourceRanges.get(merged[j2]);
        if (!range) break;
...
function stripDanglingHtmlLikeTail(markdown) {
  ...
  const isInsideFencedCodeBlock = (src, pos) => {
    ...
    const parseFenceMarker = (line) => {
      ...
      let j2 = i2;
      while (j2 < line.length && line[j2] === ch) j2++;
      const len = j2 - i2;
    ...
    for (const line of lines) {
      ...
      const lineEnd$2 = offset + line.length;
...
function normalizeCustomHtmlOpeningTagSameLine(markdown, tags) {
  ...
  const isNameChar = (ch) => {
    const c2 = ch.charCodeAt(0);
    return c2 >= 65 && c2 <= 90 || c2 >= 97 && c2 <= 122 || c2 >= 48 && c2 <= 57 || ch === "_" || ch === "-";
  };
  const trimStartIndentWs = (s) => {
    let i2 = 0;
    while (i2 < s.length && isIndentWs(s[i2])) i2++;
    return s.slice(i2);
  };
  const findTagCloseIndexOutsideQuotes$1 = (input) => {
    let inSingle = false;
  ...
  const hasClosingTagOnLine = (line, from, tag) => {
    ...
    while (pos !== -1) {
      ...
      let matched = true;
      for (let j2 = 0; j2 < lowerTag.length; j2++) {
        const ch = line[i2 + j2];
        if ((ch >= "A" && ch <= "Z" ? String.fromCharCode(ch.charCodeAt(0) + 32) : ch) !== lowerTag[j2]) {
          matched = false;
          break;
        }
      }
      ...
      let k2 = i2 + lowerTag.length;
      if (k2 < line.length && isNameChar(line[k2])) {
        pos = line.indexOf("<", pos + 1);
        continue;
      }
      while (k2 < line.length && isIndentWs(line[k2])) k2++;
      if (k2 < line.length && line[k2] === ">") return true;
      pos = line.indexOf("<", pos + 1);
    }
    return false;
  };
...
function ensureBlankLineAfterCustomHtmlCloseBeforeBlockMarkerSameLine(markdown, tags) {
  ...
  while (idx < markdown.length) {
    ...
    if (!inFence && nextContent.includes("</")) for (const re of closeTagRes) nextContent = nextContent.replace(re, (match2, closeTag, offset, src) => {
      var _a2, _b, _c, _d;
      if (src.replace(/^[\t ]+/, "").startsWith("|")) return match2;
      const before = src.slice(0, offset).replace(/^[\t ]+/, "");
      if (before.length > 0) {
        const closeTagName = ((_b = (_a2 = closeTag.match(/^<\s*\/\s*([A-Z][\w:-]*)/i)) == null ? void 0 : _a2[1]) == null ? void 0 : _b.toLowerCase()) ?? "";
  ...
  while (idx < markdown.length) {
    ...
    if (!inFence) {
      ...
      if (tag) {
        ...
        } else for (let j2 = customBlockStack.length - 1; j2 >= 0; j2--) if (customBlockStack[j2] === tag.name) {
          customBlockStack.length = j2;
          break;
        }
      }
    }
...
function parseMarkdownToStructure(markdown, md, options = {}) {
  ...
  if (((_a2 = options.customHtmlTags) == null ? void 0 : _a2.length) && safeMarkdown.includes("<")) {
  ...
  const validateLink$1 = options.validateLink ?? directValidateLink ?? ((_b = mdAny.options) == null ? void 0 : _b.validateLink) ?? (typeof mdAny.validateLink === "function" ? mdAny.validateLink : void 0);
...
function processTokens(tokens, options) {
  ...
  let i2 = 0;
  while (i2 < tokens.length) {
    const handled = parseCommonBlockToken(tokens, i2, linkifyContext.options(), containerTokenHandlers);
    if (handled) {
      recordInternalNodeSourceRange(handled[0], tokens[i2], options);
    ...
    switch (token.type) {
      case "paragraph_open": {
        const paragraphRaw = String(((_a2 = tokens[i2 + 1]) == null ? void 0 : _a2.content) ?? "");
        const paragraphNode = parseParagraph(tokens, i2, linkifyContext.options(paragraphRaw));
        if (includeSourceMap) applyNodeSourceMap(paragraphNode, token, options);
      ...
      case "bullet_list_open":
      case "ordered_list_open": {
        const [listNode, newIndex] = parseList(tokens, i2, linkifyContext.options());
      ...
      case "blockquote_open": {
        const [blockquoteNode, newIndex] = parseBlockquote(tokens, i2, linkifyContext.options());
      ...
      case "inline":
        {
          ...
          if (parsed.length === 0) {
          } else if (parsed.every((n2) => n2.type === "html_block")) {
...
var HTML_TOKEN_TAG_NAME_RE = /^([a-z][\w-]*)(?=[\t\n\f\r />]|$)/i;
...
function parseHtmlTokenTag(rawContent) {
  ...
  const match2 = source.match(HTML_TOKEN_TAG_NAME_RE);
  if (!match2) return null;
  return {
    attrsStr: isClosing ? "" : source.slice(match2[0].length).trimStart(),
    isClosing,
...
var FOREIGN_OBJECT_IGNORED_TAGS = /* @__PURE__ */ new Set([
  "script",
  "style",
  "iframe",
  "object",
...
var URL_LIKE_SVG_ATTRS = /* @__PURE__ */ new Set([
  "href",
  "xlink:href",
  "src",
  "srcdoc",
...
var URL_REFERENCE_SVG_ATTRS = /* @__PURE__ */ new Set([
  "clip-path",
  "fill",
  "filter",
  "marker-end",
...
var RENDERABLE_SVG_TAGS = /* @__PURE__ */ new Set([
  "circle",
  "ellipse",
  "image",
  "line",
...
function hasSafeSvgHref(node) {
  var _a2;
  return ((_a2 = node.getAttribute("href") || node.getAttribute("xlink:href")) == null ? void 0 : _a2.startsWith("#")) === true;
}
...
function isRenderableSvgNode(node) {
  ...
  if (tag === "text" || tag === "tspan") return Boolean((_a2 = node.textContent) == null ? void 0 : _a2.trim());
...
function hardenSvgAnchorAttrs(node) {
  ...
  if (((_a2 = node.getAttribute("target")) == null ? void 0 : _a2.trim().toLowerCase()) !== "_blank") return;
...
function replaceForeignObjectLabels(svgEl) {
  ...
  for (const node of Array.from(svgEl.querySelectorAll("foreignObject"))) {
    ...
    const x2 = parseSvgNumber(node.getAttribute("x"));
    const y2 = parseSvgNumber(node.getAttribute("y"));
    const text$1 = svgEl.ownerDocument.createElementNS(SVG_NS, "text");
    text$1.setAttribute("x", String(x2 + width / 2));
    text$1.setAttribute("y", String(y2 + height / 2));
    text$1.setAttribute("text-anchor", "middle");
    text$1.setAttribute("dominant-baseline", "central");
    const label = node.querySelector(".nodeLabel");
    if (label == null ? void 0 : label.getAttribute("class")) text$1.setAttribute("class", label.getAttribute("class"));
    if (lines.length === 1) text$1.textContent = lines[0];
      ...
      for (const [index, line] of lines.entries()) {
        ...
        tspan.setAttribute("x", String(x2 + width / 2));
        tspan.setAttribute("dy", index === 0 ? `${firstDy}em` : "1.2em");
        tspan.textContent = line;
        text$1.appendChild(tspan);
      }
    }
    (_a2 = node.parentNode) == null ? void 0 : _a2.replaceChild(text$1, node);
  }
}
...
function sanitizeMermaidSvg(svg) {
  var _a2;
  return ((_a2 = toSafeSvgElement(svg)) == null ? void 0 : _a2.outerHTML) ?? null;
}
...
function getMarkdown(msgId = `editor-${Date.now()}`, options = {}) {
  ...
  mdState.__markstreamHasCustomParserExtensions = Boolean(((_a2 = options.plugin) == null ? void 0 : _a2.length) || ((_b = options.apply) == null ? void 0 : _b.length) || _registeredMarkdownPlugins.length);
  ...
  if (Array.isArray(options.plugin)) for (const p2 of options.plugin) {
    const pluginItem = p2;
  ...
  if (_registeredMarkdownPlugins.length) {
    for (const p2 of _registeredMarkdownPlugins) if (Array.isArray(p2)) {
      const [fn, ...params] = p2;
      if (typeof fn === "function") md.use(fn, ...params);
    } else if (typeof p2 === "function") md.use(p2);
  }
  md.use(sub_plugin);
  md.use(sup_plugin);
  md.use(ins_plugin$1);
  ...
  md.core.ruler.after("block", "mark_fence_closed", (state) => {
    ...
    const s = state;
    const src = s.src;
    const envFinal = !!((_a3 = s.env) == null ? void 0 : _a3.__markstreamFinal);
    const lines = src.split(/\r?\n/);
    for (const token of s.tokens) {
      ...
      const closed = envFinal ? true : endLine > openLine + 1 && count >= minLen && j2 === line.length;
  ...
  md.renderer.rules.fence = (tokens, idx) => {
    ...
    const copyLabel = escapeHtml(t2("common.copy"));
  ...
  const referenceInline = (state, silent) => {
    ...
    const match2 = RE_REFERENCE.exec(s.src.slice(s.pos));
    if (!match2) return false;
    const lookbehind = s.src.slice(Math.max(0, s.pos - 120), s.pos);
    if (/"[^"\n]{1,80}"\s*:\s*$/.test(lookbehind)) return false;
    const afterMatch = s.src.slice(s.pos + match2[0].length);
...
// ../../node_modules/.pnpm/markstream-vue@1.0.9-beta.1_katex@0.17.0_mermaid@11.15.0_stream-diffs@0.0.2_@shikijs+th_f2c92339b182368d2c962735813644e7/node_modules/markstream-vue/dist/index2.js
var t = (t2, s, v) => new Promise((l2, a2) => {
  var h2 = (t3) => {
    try {
      n2(v.next(t3));
    } catch (s2) {
  ...
  }, n2 = (t3) => t3.done ? l2(t3.value) : Promise.resolve(t3.value).then(h2, e2);
  n2((v = v.apply(t2, s)).next());
});
var l = Symbol.for("markstream-vue:node-lifecycle");
function a() {
  return inject(l, null);
}
var h = false;
function e() {
  return h;
...
function g(t2) {
  if (!c.has(t2)) throw new Error(`[markstream-vue] Unknown icon theme: "${t2}". Registered: ${[...c.keys()].join(", ")}`);
  i = t2, null == w || w();
  const s = c.get(t2);
  s.loadExtended && !o.has(t2) && u(s);
}
function m() {
  return [...c.keys()];
}
...
function B(t2) {
  ...
  const v = (function(t3) {
    ...
    const s2 = t3.trim();
    if (!s2) return "";
    const [v2] = s2.split(/\s+/), [l2] = v2.split(":");
    return l2.toLowerCase();
  })(t2);
  return null != (s = j[v]) ? s : v;
}
function V(t2) {
  const s = B(t2);
...
function b() {
  return t(this, null, function* () {
    yield (function() {
      return t(this, null, function* () {
        const t2 = c.get(i);
        (null == t2 ? void 0 : t2.loadExtended) && (yield u(t2));
      });
    })();
  });
}
...
var C = { js: "JavaScript", javascript: "JavaScript", ts: "TypeScript", jsx: "JSX", tsx: "TSX", html: "HTML", css: "CSS", scss: "SCSS", json: "JSON", py: "Python", python: "Python", rb: "Ruby", go: "Go", java: "Java", c: "C", cpp: "C++", cs: "C#", csharp: "C#", php: "PHP", sh: "Shell", bash: "Bash", sql: "SQL", yaml: "YAML", md: "Markdown", d2: "D2", d2lang: "D2", "": "Plain Text", plain: "Plain Text" };
function k() {
  return t(this, null, function* () {
    const { preloadCodeBlockRuntime: t2 } = yield import("./monaco-SG4FY46U.js");
    return t2();
  });
}
function L() {
  return t(this, null, function* () {
    const { getUseMonaco: t2 } = yield import("./monaco-SG4FY46U.js");
    return t2();
  });
}
...
//# sourceMappingURL=chunk-TJLASVOG.js.map
