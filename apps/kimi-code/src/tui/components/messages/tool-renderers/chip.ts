...
import { webSearchResultCount } from './web-search';
...
const webSearchChip: ChipProvider = (_toolCall, result) => {
  const count = webSearchResultCount(result.output);
  if (count > 0) return pluralize(count, 'result');
  const lines = result.output.split('\n').filter((l) => l.trim().length > 0);
  return lines.length === 0 ? 'no results' : 'web result';
};
