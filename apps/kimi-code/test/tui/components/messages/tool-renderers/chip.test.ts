...
describe('chip registry', () => {
  ...
  it('WebSearch chip shows result count from Title blocks', () => {
    expect(
      chipFor(
        'WebSearch',
        { query: 'kimi' },
  ...
  it('WebSearch chip says "no results" on empty output', () => {
    expect(chipFor('WebSearch', { query: 'kimi' }, result(''))).toBe('no results');
  });
