...
describe('tool-result registry', () => {
  ...
  it('WebSearch renders a parsed preview with title, host, URL and snippet', () => {
    ...
    expect(out).not.toContain('Fourth result');
    expect(out).toContain('... (1 more result, ctrl+o to expand)');
  });

  it('WebSearch expands to all results with longer snippets', () => {
    const renderer = pickResultRenderer('WebSearch');
    const out = strip(
      joinRender(
        renderer(
  ...
  it('WebSearch falls back to truncated renderer for unparseable output', () => {
    const renderer = pickResultRenderer('WebSearch');
    const out = strip(joinRender(renderer(call('WebSearch', { query: 'x' }), result('No search results found.'), ctx)));
    expect(out).toContain('No search results found.');
  });

  it('WebSearch error falls back to truncated renderer', () => {
    const renderer = pickResultRenderer('WebSearch');
    const out = strip(
      joinRender(renderer(call('WebSearch', { query: 'x' }), result('Search failed (network)', true), ctx)),
    );
    expect(out).toContain('Search failed (network)');
  });
