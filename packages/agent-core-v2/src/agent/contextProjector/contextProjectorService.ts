...
export class AgentContextProjectorService implements IAgentContextProjectorService {
  ...
  downgradeUnsupportedMedia(
    ...
    return downgradeUnsupportedMedia(messages, capability);
  }
...
const CAPABILITY_PLACEHOLDERS = {
  image_url: '[image omitted: current model has no image input]',
  audio_url: '[audio omitted: current model has no audio input]',
  video_url: '[video omitted: current model has no video input]',
} as const;
...
export function downgradeUnsupportedMedia(
  ...
  if (isUnknownCapability(capability)) return messages;
  const dropImage = !capability.image_in;
  const dropAudio = !capability.audio_in;
  const dropVideo = !capability.video_in;
  if (!dropImage && !dropAudio && !dropVideo) return messages;

  let changed = false;
  const result = messages.map((message) => {
    let messageChanged = false;
    const content = message.content.map((part): ContentPart => {
      if (!isDegradableMediaPart(part)) return part;
      const shouldDrop =
        (part.type === 'image_url' && dropImage) ||
        (part.type === 'audio_url' && dropAudio) ||
        (part.type === 'video_url' && dropVideo);
      if (!shouldDrop) return part;
      changed = true;
      messageChanged = true;
      return { type: 'text', text: CAPABILITY_PLACEHOLDERS[part.type] };
    });
    return messageChanged ? { ...message, content } : message;
  });
  return changed ? result : messages;
