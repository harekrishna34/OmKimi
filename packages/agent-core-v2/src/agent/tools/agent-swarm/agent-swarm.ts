...
export const AgentSwarmToolInputSchema = z
  .object({
    ...
    model: z
      ...
      .optional()
      .describe(
        'Which model to run the item-spawned subagents on: "secondary" = the configured secondary model; "primary" = the main model you are running on; or any configured [models] alias (e.g. from the subagent model list) to bind them to that model directly. This explicit choice overrides the selected agent type\'s model_preference; without one, secondary is the default when configured, otherwise subagents inherit your model. Resumed subagents always keep their own model.',
      ),
    models: z
      .array(z.string().trim().min(1))
      .max(MAX_AGENT_SWARM_SUBAGENTS)
      .optional()
      .describe(
        'Per-item model for each new subagent spawned from items, aligned by index with items. Each entry follows the same rules as the model parameter ("secondary", "primary", or any configured [models] alias). Overrides model for the matching spawn; entries beyond items are ignored.',
      ),
  })
  .strict();
