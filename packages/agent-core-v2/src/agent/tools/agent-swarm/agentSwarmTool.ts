...
export class AgentSwarmTool implements IAgentSwarmTool {
  ...
  private async runSwarm(
    ...
    const hasSpawns = (args.items?.length ?? 0) > 0;
    let resolvedOwn: { modelAlias: string; thinkingLevel: string } | undefined;
    let defaultPreference: string | undefined;
    if (hasSpawns) {
      ...
      defaultPreference = targetProfile.modelPreference;
      if (own.modelAlias !== undefined) {
        resolvedOwn = { modelAlias: own.modelAlias, thinkingLevel: own.thinkingLevel };
      }
    }
    const resolveBinding = (requested: string | undefined) => {
      if (resolvedOwn === undefined) return undefined;
      return resolveSubagentBinding(
        this.config,
        this.flags,
        resolvedOwn,
        requested ?? args.model ?? defaultPreference,
      );
    };
    ...
    const tasks: SessionSwarmTask<AgentSwarmSpec>[] = specs.map((spec) => {
      ...
      const resolved = resolveBinding(spec.model);
      const binding = resolved === undefined ? undefined : { model: resolved.model, thinking: resolved.thinking };
      return {
        ...common,
        kind: 'spawn' as const,
        binding,
...
async function createAgentSwarmSpecs(
  ...
  if (items.length > 0) {
    ...
    const itemModels = (args.models ?? []).map((model) => normalizeOptionalString(model));
