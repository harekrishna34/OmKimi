...
export const SubagentToolInputSchema = z.preprocess(
  ...
  z.object({
    ...
    model: z
      ...
      .optional()
      .describe(
        'Which model to run the subagent on: "secondary" = the configured secondary model; "primary" = the main model you are running on; or any configured [models] alias (e.g. from the subagent model list) to bind this subagent to that model directly. This explicit choice overrides the selected agent type\'s model_preference; without one, secondary is the default when configured, otherwise the subagent inherits your model. Ignored when resuming — resumed subagents keep their own model.',
      ),
  }),
);
