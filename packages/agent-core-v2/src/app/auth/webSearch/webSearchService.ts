...
export class WebSearchProviderService implements IWebSearchProviderService {
  ...
  private fromServicesConfig(): WebSearchProvider | undefined {
    ...
    if (isTavilyBaseUrl(search.baseUrl)) {
...
function isTavilyBaseUrl(baseUrl: string): boolean {
  let host = '';
  try {
  ...
  return host === 'api.tavily.com' || host.endsWith('.tavily.com');
}
