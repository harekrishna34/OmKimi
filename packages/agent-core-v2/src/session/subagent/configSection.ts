...
export const SUBAGENT_MODELS_ENV = 'KIMI_SUBAGENT_MODELS';

const SubagentModelsConfigSchema = z.object({
  models: z.array(z.string().min(1)).optional(),
});

export type SubagentModelsConfig = z.infer<typeof SubagentModelsConfigSchema>;

function parseModelListEnv(raw: string): string[] | undefined {
  const models = raw
    .split(',')
...
export const subagentModelsEnvBindings: EnvBindings<SubagentModelsConfig> = envBindings(
  SubagentModelsConfigSchema,
  {
    models: { env: SUBAGENT_MODELS_ENV, parse: parseModelListEnv },
  },
);

registerConfigSection(SUBAGENT_MODELS_SECTION, SubagentModelsConfigSchema, {
  env: subagentModelsEnvBindings,
  stripEnv: stripEnvBoundFields(subagentModelsEnvBindings),
});

export type SubagentModelChoice = 'primary' | 'secondary' | string;

const SECONDARY_CHOICE = 'secondary';
const PRIMARY_CHOICE = 'primary';

function normalizeOptionalChoice(value: string | undefined): string | undefined {
...
export function resolveSubagentBinding(
  ...
  const choice = normalizeOptionalChoice(requested);
  if (choice !== undefined && choice !== PRIMARY_CHOICE && choice !== SECONDARY_CHOICE) {
    return {
      model: choice,
      thinking: undefined,
  ...
  if (choice !== PRIMARY_CHOICE && secondary?.model !== undefined) {
    const model =
...
export function buildSubagentModelDescriptions(
  ...
  const configured =
    config.get<SubagentModelsConfig | undefined>(SUBAGENT_MODELS_SECTION)?.models ?? [];
  ...
  for (const model of configured) {
    if (model === secondaryModel || model === callerModelAlias) continue;
    lines.push(
      `- ${model} — a configured subagent model; pass it by name to run this subagent on it${capabilitiesSuffix(resolvedCapabilities(modelCatalog, model))}`,
    );
  }
  lines.push(
  ...
  lines.push(
    'Any configured [models] alias also works: pass it as model to bind this subagent to it directly.',
  );
  return lines.join('\n');
}
