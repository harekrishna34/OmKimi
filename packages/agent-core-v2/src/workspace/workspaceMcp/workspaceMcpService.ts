...
export class WorkspaceMcpService extends Service implements IWorkspaceMcpService {
  ...
  private readonly initialLoadServers = new Set<string>();
  ...
  private sessionBaseline(
    ...
    void ready.then(
      () => {
        // Finalize the baseline with the workspace's initial-load server set.
        // A handle requested before the initial load completed (e.g. the very
        // first session racing the workspace materialization) would otherwise
        // freeze with an incomplete snapshot and never attach those servers'
        // tools, even though they were part of the load.
        for (const name of this.initialLoadServers) {
          baseline.add(name);
        }
        frozen = true;
      },
  ...
  private async initialize(): Promise<void> {
    ...
    for (const name of Object.keys(servers)) {
      this.initialLoadServers.add(name);
    }
