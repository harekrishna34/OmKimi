...
describe('projector tool-exchange normalization', () => {
  ...
  describe('downgradeUnsupportedMedia', () => {
    function mediaMessage(url: string, id?: string): ContextMessage {
      return {
        role: 'user',
        content: [{ type: 'image_url', imageUrl: { url, id } }],
        toolCalls: [],
    ...
    it('replaces media the model cannot consume with a text marker', () => {
      const projected = projector.downgradeUnsupportedMedia(
        projector.project([
          user('look at this'),
          mediaMessage('data:image/png;base64,AAAA'),
          {
            role: 'user',
            content: [{ type: 'video_url', videoUrl: { url: 'data:video/mp4;base64,BBBB' } }],
            toolCalls: [],
            origin: { kind: 'user' },
          },
        ]),
        { image_in: false, video_in: true, audio_in: true, thinking: true, tool_use: true, max_context_tokens: 128000 },
      );
      ...
      const texts = allParts.filter((part) => part.type === 'text').map((part) => part.text);
      expect(texts).toContain('look at this');
      expect(texts).toContain('[image omitted: current model has no image input]');
    });

    it('drops audio and video when the model lacks those inputs', () => {
      const projected = projector.downgradeUnsupportedMedia(
        projector.project([
          mediaMessage('data:image/png;base64,AAAA'),
          {
            role: 'user',
            content: [
              { type: 'audio_url', audioUrl: { url: 'data:audio/wav;base64,CCCC' } },
        ...
        { image_in: true, video_in: false, audio_in: false, thinking: true, tool_use: true, max_context_tokens: 128000 },
      );

      const allParts = projected.flatMap((message) => message.content);
      expect(allParts.some((part) => part.type === 'image_url')).toBe(true);
      expect(allParts.some((part) => part.type === 'audio_url')).toBe(false);
      expect(allParts.some((part) => part.type === 'video_url')).toBe(false);
      const texts = allParts.filter((part) => part.type === 'text').map((part) => part.text);
      expect(texts).toContain('[audio omitted: current model has no audio input]');
      expect(texts).toContain('[video omitted: current model has no video input]');
    });

    it('leaves media untouched when the model supports every present modality', () => {
      const input = projector.project([
        user('look at this'),
        mediaMessage('data:image/png;base64,AAAA'),
      ]);
      const projected = projector.downgradeUnsupportedMedia(
        input,
        { image_in: true, video_in: true, audio_in: true, thinking: true, tool_use: true, max_context_tokens: 128000 },
      );
