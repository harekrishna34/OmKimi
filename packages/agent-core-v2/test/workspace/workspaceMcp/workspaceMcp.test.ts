...
describe('WorkspaceMcpService', () => {
  ...
  it('sessionHandle finalizes the baseline with initial-load servers once ready settles', async () => {
    current = { alpha: stdioServer() };
    let settleConnectAll: () => void = () => undefined;
    vi.spyOn(McpConnectionManager.prototype, 'connectAll').mockImplementation(function (
      this: McpConnectionManager,
    ...
    const service = createService();
    manager = service.connectionManager();
    // The handle is requested before the initial load has materialized any
    // server — e.g. the first session racing workspace materialization. No
    // status-change subscription runs during the window (no agent yet), so the
    // lazy admission path never populates the baseline.
    const handle = service.sessionHandle();
    ...
    // Even frozen without a prior admission, the initial-load server is
    // baseline, so a later agent still attaches its tools.
    expect(handle.isBaselineServer('alpha')).toBe(true);
    expect(handle.isBaselineServer('ghost')).toBe(false);
  }, 20000);
