...
describe('WebSearchProviderService', () => {
  ...
  it('builds a Tavily search provider when the moonshot_search baseUrl is api.tavily.com', async () => {
    servicesConfig = {
      moonshotSearch: {
  ...
  it('builds a Tavily provider for a .tavily.com host with the default query body', async () => {
    servicesConfig = {
      moonshotSearch: {
        baseUrl: 'https://search.tavily.com',
        apiKey: 'tvly-test',
      },
    };
    const fetchMock = vi.fn().mockResolvedValue({
    ...
    const [url, init] = fetchMock.mock.calls[0] as [string, RequestInit];
    expect(url).toBe('https://search.tavily.com');
    expect(JSON.parse(init.body as string)).toEqual({ query: 'hello' });
  });

  it('builds a Moonshot provider for a non-Tavily baseUrl', async () => {
    servicesConfig = {
      moonshotSearch: {
        baseUrl: 'https://search.example.com/search',
        apiKey: 'search-key',
  ...
  it('surfaces Tavily HTTP 401 as a coded search failure', async () => {
    servicesConfig = {
      moonshotSearch: { baseUrl: 'https://api.tavily.com/search', apiKey: 'tvly-test' },
    };
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({
        status: 401,
        text: async () => 'Unauthorized: missing or invalid API key.',
      }),
    );

    const provider = createService().getWebSearchProvider();
    await expect(provider!.search('hello')).rejects.toMatchObject({
      code: ErrorCodes.WEB_FETCH_FAILED,
    });
  });

  it('answers presence for a Tavily-configured service', () => {
    servicesConfig = {
      moonshotSearch: { baseUrl: 'https://api.tavily.com/search', apiKey: 'tvly-test' },
    };
...
describe('services config section', () => {
  ...
  it('maps Tavily search options from TOML snake_case to camelCase', () => {
    expect(
      servicesFromToml({
        moonshot_search: {
