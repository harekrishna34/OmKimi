...
function createModelCatalogStub(): IModelCatalog {
  return {
...
describe('subagent config section', () => {
  ...
  it('reads the subagent model allowlist from [subagent_models] and env, and advertises it', async () => {
    const fromToml = await createConfig({}, '[subagent_models]\nmodels = ["provider/alpha", "provider/beta"]\n');
    expect(fromToml.config.get(SUBAGENT_MODELS_SECTION)).toEqual({
      models: ['provider/alpha', 'provider/beta'],
    });
    fromToml.disposables.dispose();

    const withEnv = await createConfig({ [SUBAGENT_MODELS_ENV]: ' provider/alpha , provider/gamma ,' });
    expect(withEnv.config.get(SUBAGENT_MODELS_SECTION)).toEqual({
      models: ['provider/alpha', 'provider/gamma'],
    });
    withEnv.disposables.dispose();

    const modelCatalog = createModelCatalogStub();
    const own = { modelAlias: 'provider/main', thinkingLevel: 'medium' };
    const { config, disposables } = await createConfig(
      {},
      '[subagent_models]\nmodels = ["provider/alpha", "provider/main"]\n[secondary_model]\nmodel = "provider/secondary"\n',
    );
    const lines = buildSubagentModelDescriptions(
      config,
      secondaryModelFlags(),
      own.modelAlias,
      modelCatalog,
    );
    expect(lines).toContain('- secondary: provider/secondary (default)');
    expect(lines).toContain('- provider/alpha');
    expect(lines).toContain('- primary: provider/main');
    expect(lines).not.toContain('- provider/main — a configured subagent model');
    disposables.dispose();
  });

  it('advertises only the primary model when nothing else is configured', async () => {
    const { config, disposables } = await createConfig({});
    const modelCatalog = createModelCatalogStub();
    const lines = buildSubagentModelDescriptions(
      config,
    ...
    expect(lines).toContain('- primary: provider/main');
    expect(lines).not.toContain('- secondary:');
    disposables.dispose();
  });
