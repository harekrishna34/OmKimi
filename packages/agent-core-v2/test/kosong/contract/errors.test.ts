...
describe('isImageFormatError', () => {
  it('recognizes the provider deserialization rejection for an unsupported image part', () => {
    const error = new APIStatusError(
      400,
      'Failed to deserialize the JSON body into the target type: ' +
        'messages[11]: unknown variant `image_url`, expected `text` at line 1 column 389300',
    );
    expect(isImageFormatError(error)).toBe(true);
  });

  it('does not treat other deserialization failures as image format errors', () => {
    expect(
      isImageFormatError(
        new APIStatusError(400, 'Failed to deserialize the JSON body: expected a list'),
      ),
    ).toBe(false);
  });

  it('does not match non-400 status errors', () => {
    expect(
      isImageFormatError(
        new APIStatusError(500, 'unknown variant `image_url`, expected `text`'),
      ),
    ).toBe(false);
  });
});
