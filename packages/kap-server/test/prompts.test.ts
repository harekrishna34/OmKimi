...
describe('server-v2 /api/v1 prompts', () => {
  ...
  async function mainAgent(sessionId: string) {
    const session = getLiveSessionById(server!.core.accessor, sessionId);
    if (session === undefined) throw new Error(`session ${sessionId} not found`);
    const agent = session.accessor.get(IAgentLifecycleService).get('main');
    if (agent === undefined) throw new Error(`main agent ${sessionId} not found`);
    return agent;
  }
  ...
  it('auto-shifts to a media-capable model when the prompt carries an image', async () => {
    const id = await createSession(home as string);
    await createMainAgent(id);
    const agent = await mainAgent(id);

    // Bind the default (text-only) model first, then submit an image prompt
    // without an explicit model — the route must shift to the capable model.
    ...
    const submitted = await call<PromptItemWire>('POST', `/api/v1/sessions/${id}/prompts`, {
      content: [
        ...
        { type: 'image', source: { kind: 'url', url: 'https://example.test/red.png' } },
      ],
    });
    expect(submitted.body.code).toBe(0);

    expect(profile.data().modelAlias).toBe('vision');
  });

  it('leaves the model untouched for text-only prompts', async () => {
    const id = await createSession(home as string);
    await createMainAgent(id);
    const agent = await mainAgent(id);
