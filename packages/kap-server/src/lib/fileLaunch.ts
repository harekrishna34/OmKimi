...
export async function launchDetached(cmd: LaunchCommand): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    ...
    child.once('error', (err) => {
      if (settled) return;
      settled = true;
      // Best-effort open/reveal: if the platform command isn't installed
      // (e.g. xdg-open on a headless container), don't surface a 50001 —
      // the file simply doesn't open visually.
