...
type MediaCapability = 'image_in' | 'audio_in' | 'video_in';

const MEDIA_PART_TO_CAPABILITY: Readonly<Record<string, MediaCapability>> = {
  image_url: 'image_in',
  audio_url: 'audio_in',
  video_url: 'video_in',
};
...
/**
 * Auto-shift the session's model to one that can accept the prompt's media
 * before the turn runs. When the content carries an `image_url` / `audio_url`
 * / `video_url` part the currently bound model cannot accept, pick a configured
 * model that declares the matching capability and bind it. Selection prefers
 * `KIMI_MEDIA_MODEL` when set, else the first configured capable model (model
 * order in config). A no-op when nothing needs shifting or no capable model is
 * configured. This is the proactive counterpart to the engine's
 * `downgradeUnsupportedMedia`: media reaches a capable model instead of being
 * placeholder-downgraded.
 */
async function shiftModelToMediaCapable(
  profile: IAgentProfileService,
  core: Scope,
  parts: readonly ContentPart[],
): Promise<void> {
  const required = new Set<MediaCapability>();
  for (const part of parts) {
    const capability = MEDIA_PART_TO_CAPABILITY[part.type];
    if (capability !== undefined) required.add(capability);
  }
  if (required.size === 0) return;

  const current = profile.data();
  const currentCapabilities = current.modelCapabilities;
  let missing = false;
  ...
  if (current.modelAlias !== undefined && !missing) return;

  const catalog = core.accessor.get(IModelCatalog);
  const items = await catalog.listModels();
  if (items.length === 0) return;

  const capable = items.filter((item) => {
  ...
  const preferredEnv = process.env['KIMI_MEDIA_MODEL'];
  const preferred =
    preferredEnv !== undefined ? capable.find((item) => item.model === preferredEnv) : undefined;
  const target = preferred ?? capable[0];
  if (target === undefined) return;

  await profile.setModel(target.model);
}
  ...
  const submitRoute = defineRoute(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        await shiftModelToMediaCapable(resolved.profile, core, parts);
